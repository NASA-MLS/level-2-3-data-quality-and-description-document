...
\texttt{L2gpPrecision}) which is the \emph{a posteriori} precision as returned by
...
\item There is a tendency for negative values to occur at the level
  ...
  middle atmosphere.
\item As discussed above, the \thisv CO retrievals are
  sensitive to the presence of thick clouds, and the screening procedure
  described above based on \texttt{Status}, \texttt{Quality} and
  \texttt{Convergence} should be applied before any scientific
  application of the MLS CO data at pressures greater than 100\,hPa.
\end{itemize}
