...
The Level~3 products can be divided into separate families with two
time resolutions, daily and monthly.  The daily files are stored one
day per file, with select families of data also stored in a year file
with 365 or 366 entries per file.  The monthly files are stored in
yearly files with 12 entries per file.
...
\item[Daily and monthly 3D grids:] Latitude/Longitude/Pressure and
  ...
  These are only stored in the daily files (one entry/day per file) and
  monthly files (12 entries/months per file).
\item[Daily and monthly latitude/pressure and
  latitude/potential-temperature zonal means:] Stored in both daily
  and annual files for each product, with one entry per day for the
  daily files, 365 or 366 entries per year for the yearly files, and
  12 entries per year for the monthly files.
...
fields except for the daily gridded (latitude/longitude families),
which are not included for file size considerations.  The
\texttt{L3SY} files contain 365 or 366 time entries as appropriate.
