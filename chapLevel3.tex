...
potential temperature windows).  This choice has implications for the
\texttt{minimum} and \texttt{maximum} fields described
below. \mycomment{Think/discuss how it affects the standard deviation
  also?}
...
provided---daily, and monthly.  All the daily and monthly zonal mean,
equivalent latitude/potential temperature mean, and vortex average
fields are stored in files having yearly granularity (i.e., one file
per year per Level~2 data product, with either 365, 366, or 12 time
entries).  These files also contains the monthly 3D-gridded (vertical,
latitude, longitude) fields.  The fields with daily temporal
resolution are in \texttt{L3SY} files, while those with monthly
resolution are in \texttt{L3SM} files. Given their size, the daily
3D-gridded (vertical, latitude, longitude) fields are stored in
individual daily \texttt{L3SD} files.  For completeness, these daily
files also contain the same zonal mean, etc.\ fields included in the
\texttt{L3SY} products.

\mycomment{Ryan's original: For the Level 3 data products, there are 3
  time granularities: daily (\texttt{L3SD}), monthly (\texttt{L3SM}),
  and yearly (\texttt{L3SY}).  Individual \texttt{L3SD} files are
  available for each day for each data product, while the
  \texttt{L3SM} and \texttt{L3SY} data are separated into yearly files
  for each product \mycomment{Include discussion of how we'll
    generate, handle and distribute results for the incoming year}.
  The \texttt{L3SD} and \texttt{L3SM} files contain data for their
  corresponding days and months.  The \texttt{L3SY} files are a
  conglomeration of all the daily zonal mean and vortex average
  quantities that are reported in the \texttt{L3SD} files.  These
  provided to facilitate analyses of daily timeseries.}
...
and monthly files contain daily grids, \mycomment{Does that mean the
  monthly files contain 30 daily grids too (in which case I need to
  change my take on Ryan's paragraph, and does that imply one file per
  month?)? or should this be ``daily/monthly'' grids?} while all files
contain zonal mean and vortex average groups.  These are named as the
following where product is the same as listed in the filename:
...
\vspace{\baselineskip}\noindent\begin{tabular}{>{\ttfamily}r>{\raggedright}p{4.9in}}
  ...
  rms\_uncertainty: & The RMS precision of each bin\mycomment{Has this
    been divided by $\sqrt{n}$?}\tabularnewline
...
considering the \texttt{nvalues} field \mycomment{unify with list of
  fields, above}).  In contrast, the ``accuraccy''-related errors,
those due to biases in the MLS measurement system, will in most cases
be propagated through the averaging process unchanged, and should be
assumed to apply to the Level~3 values in the same manner as they do
in Level~2 datasets.
