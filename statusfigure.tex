...
\node (m3) at (\xpos,\ypos+3\meaningheight) {\meaningbox{Information:}
  {(Bad profile) This profile has been manually marked bad by the MLS science team.}};
\node (m4) at (\xpos,\ypos+4\meaningheight) {\meaningbox{Information:}%
  {(Warning) This profile may have been affected by high-altitude clouds.}};
\node (m5) at (\xpos,\ypos+5\meaningheight) {\meaningbox{Information:}%
  {(Warning) This profile may have been affected by low-altitude clouds.}};
\node (m6) at (\xpos,\ypos+6\meaningheight) {\meaningbox{Information:}%
  {(Comment) GEOS-5 \mbox{a priori} temperature data were unavailable for this profile.}};
\node (m7) at (\xpos,\ypos+7\meaningheight) {\meaningbox{Information:}%
  {(Bad profile) The retrieval for this phase encountered a numerical error.}};
\node (m8) at (\xpos,\ypos+8\meaningheight) {\meaningbox{Information:}%
  {(Bad profile) Too few radiances were available for good retrieval of this profile.}};
\node (m9) at (\xpos,\ypos+9\meaningheight) {\meaningbox{Information:}%
  {(Bad profile) The task retrieving this profile crashed (typically a computer failure).}};
\end{scope}
%
% Now a separator
\draw[ultra thick] ($ (m2.west)!0.5!(m3.west) $) -- ($ (m2.east)!0.5!(m3.east) $);
...
\end{scope}
\end{tikzpicture}
\caption{The meaning of the various bits in the \texttt{Status}
  field.}
