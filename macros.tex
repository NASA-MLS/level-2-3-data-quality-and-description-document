...
% THIS IS THE ORIGINAL ONE FROM CHATGPT
% % ---------------------------------------- Defining a product
...
% Requires fancyhdr loaded and your header template using \rightmark, e.g.:
% \usepackage{fancyhdr}
% \pagestyle{fancy}
% \fancyhead[RO]{\nouppercase{\rightmark}} % example
% \fancyhead[LE]{\nouppercase{\leftmark}}  % report class: chapters set \leftmark
...
\newcommand{\speciestitle}[5][]{%
  ...
  }\vspace{10pt}% <-- group ends; now we are back in vertical mode
  % Update the running head mark OUTSIDE the box/group:
  \sectionmark{#2 (\texorpdfstring{#3}{#4})}%
  % If your fancyhdr template uses \rightmark directly, that's enough.
  % If you prefer to hardwire the field instead of relying on \rightmark,
  % you can uncomment the next line to set it globally for following pages:
  % \fancyhead[RO]{\large\sffamily\color{HeaderFooterColor}%
  %   \nouppercase{\thesection.\enspace #2 (#3)}}
  \renewcommand{\currentnav}{#4}%
}
