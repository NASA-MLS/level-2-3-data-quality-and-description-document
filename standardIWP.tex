...
data using the \texttt{Status} field in the collocated temperature
profile to exclude bad retrievals \citep{SchwartzEtal08}. Only IWP
values for which temperature \texttt{Status} is an even number should be
used.  In addition to the \texttt{Status} screening, the user is also
recommended to screen the IWP data for significant cloud hits on a daily
basis using the ``$2\sigma$\,--\,$3\sigma$'' method described in the IWC
section (\ref{sec:StandardIWC}). The $3\sigma$ threshold is needed for
cloud detection since a small percentage of clear-sky residual noise can
...
\begin{figure}
\includegraphics[width=\textwidth]{figures/IWP-240-v04-20-dataqual-JUL2009}
\caption{MLS v4, v3 and v2 IWP comparisons for July 2009. 
  Left: Probability density functions (PDF) (v4 (red), v3 (blue) and v2
  ...
  Scatter plot of IWP v4 vs v3 (black points) with a dashed red lined
