...
v2.2 and v3.3 / v3.4 profiles, so the stratospheric results from the
...
screening is less complex than it was for v3.3 / v3.4 data and is again
primarily based on thresholds for \texttt{Quality},and
\texttt{Convergence} as was the case for v2.2.  Ozone profile behavior
has been improved, with reduction of the vertical oscillations that
affected previous data versions in the UTLS and at low latitudes through
use of an optimized, ozone-only phase.  Improvements in the presence of
thick clouds or the upper tropospheric retrievals are accomplished
through changes in the spectral form used to model cloud impacts on the
MLS radiances, by tightening vertical smoothing at pressures greater
than or equal to 68\,hPa (leading to an increase in averaging kernel
FWHM by a factor of $\sim$1.15) and by retrieving an independent
...
\texttt{O3\ column}, which is the integrated stratospheric column using
the thermal tropopause (WMO definition) calculated from MLS temperature
as the bottom boundary.  The MLS temperature profiles from which the WMO
tropopause is determined are not screened per the instructions of
section~\ref{sec:StandardTemperature} and careful users may wish to
recalculate the stratospheric ozone column after determining the
tropopauses from screened temperature profiles.  An additional swath
containing a second stratospheric column, one integrated down to a
GEOS-5 WMO tropopause (determined from the GEOS-5 temperature profile),
was included in previous versions but not in \thisv.

\subsection*{Comparison of \thisv with past data versions}  

The vertical retrieval grid has not changed from v3.3 / v3.4; 
between 316\,hPa and 1\,hPa, \thisv ozone profiles are retrieved
on 12 surfaces per decade, a grid twice as fine as the
6-level-per-decade grid used in v2.2 Tightened vertical smoothing has
degraded vertical resolution in \thisv from the
upper troposphere to 68\,hPa by $\sim$15\%.
Table\ref{tab:O3Summary} summarizes
...
Figure~\ref{fig:MLSO3histUTLS} shows histograms of tropical UTLS v04.20 and
v03.30 ozone for all of 2005.  V04.20 ozone unscreened (blue) and
screened (green) histograms are quite similar to one another, with
only small reductions in scatter in the screened case.  V3.3 has
significantly broader distributions in the troposphere than v04.20, both in the widths of
the central peaks and in the tails of the distributions.   While
screened v3.3 (cyan) has reduced tails of cloud-impacted outliers compared to unscreened
(red), cloud sensitivity is clearly reduced in \thisv. The reduced tropical
vertical oscillation that was seen in
Figure~\ref{fig:MLSv3v4O3ContourUTLS} is reflected in the shifts of
the histogram peaks between versions, with higher \thisv values at
121\,hPa and lower values at 82, 100, 147, 178 and 316\,hPa.  The
316\,hPa level of \thisv, though not currently recommended for general
scientific use, is under evaluation. 
\begin{figure} 
...
1\%) in the new data version.  As was noted above, only one
stratospheric column swath, based upon MLS temperature, is
calculated by the \thisv O3 retrieval.  The column based upon the
GEOS-5 tropopause is no longer routinely produced. 
...
were reduced after version 3.3 / 3.4, with additional smaller
subsequent reductions in the ozone values in \thisv.  We have found
that latitudinal and temporal changes observed in various correlative
data sets are well reproduced by the MLS ozone product.
Intercomparisons of a large variety of ozone measurements by satellite
instruments have been documented by Tegtmeier et al. (2013), as part
of the analyses produced by the SPARC Data Initiative; the Aura MLS
ozone values compare quite favorably to the multi-instrument mean
values as well as to SAGE II ozone. Future validation studies will
focus more on longer-term changes and on the UTLS region, especially
in the tropics. The temporal stability of the Aura MLS ozone dataset
has been shown to be very good, in comparison to lidar datasets (see
Nair et al., 2012), as well as in other more recent work. The use of
Aura MLS data in combination with older datasets should lead to (and
has already led to) very useful investigations regarding the ozone
changes since the turn of the century (and into the ``recovery
phase'').
