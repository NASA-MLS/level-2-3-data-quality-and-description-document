...
FWHM by a factor of \texttildelow1.15) and by retrieving an independent
...
cloud-impacted outliers compared to unscreened (red), cloud sensitivity
is clearly reduced in \thisv. The reduced tropical vertical oscillation
that was seen in Figure~\ref{fig:MLSv3v4O3ContourUTLS} is reflected in
...
Changes in the MLS stratospheric ozone columns (or in columns down to
pressures between 100 and 316\,hPa) for \thisv are quite small; typical
daily zonal averages are within one percent, with a tendency towards
slightly lower values (but within about 1\%) in the new data version.
As was noted above, only one stratospheric column swath, based upon MLS
temperature, is calculated by the \thisv O\cs3 retrieval.  The column
based upon the GEOS-5 tropopause is no longer routinely produced.
...
using v4.2 data should focus on longer-term changes (for which good
...
ozonesonde and satellite datasets. Such high biases were reduced after
versions \prevv, with additional smaller subsequent reductions in the
ozone values in \thisv.  We have found that latitudinal and temporal
changes observed in various correlative data sets are well reproduced by
...
work. The use of Aura MLS data in combination with older datasets should
lead to (and has already led to) very useful investigations regarding
the ozone changes since the turn of the century (and into the ``recovery
phase'').
