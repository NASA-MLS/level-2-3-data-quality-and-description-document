...
\begin{multicols}{2}
  ...
  of version~3.3 and~3.4 EOS MLS Level~2 data.  Secondly, to convey
  important information on how to read and interpret the data to the
  ...
  (GSFC-DISC, see {\small\verb+http://disc.gsfc.nasa.gov/+}).
...
\item Do not use any data points where the precision is set negative.
  This indicates poor information yield from MLS.
...
\item Do not use any data for profiles where the \texttt{Quality} field
  is \emph{lower} than the threshold given in the section of
...
\item Do not use any data for profiles where the \texttt{Convergence}
  field is \emph{higher} than the threshold given in the section of
  chapter~\ref{chap:Results} describing your product of interest.
...
\item Information on the accuracy of each product is given in
  Chapter~\ref{chap:Results}.  Detailed MLS accuracy budgets are given
  in papers in the Aura Validation special issue of the Journal of
  Geophysical Research -- Atmospheres.  These papers describe the
  earlier (v2.2) MLS data, the accuracy budgets for which are expected
  to be similar to that of \thisv described herein.
