...
shows an example of typical differences between v3.3 and \thisv
HNO\cs3\@.  Higher values in \thisv than in v3.3 in the tropical UTLS
seen in Figure~\ref{fig:zmv3v4diffs} result primarily from the reduction
of adverse cloud effects.
...
HNO\cs3 profiles and precision estimates, along with \texttt{Quality}
and \texttt{Convergence} information from both sets of retrievals are
included in the standard HNO\cs3 files, so that the appropriate
screeening values can conveniently be used at each level.  The
\texttt{Quality} and \texttt{Convergence} included in the
``\texttt{HNO3}'' swath are from the \mbox{240-GHz} radiometer and
...
\begin{figure}
\includegraphics[angle=90,width=\linewidth]{figures/InspectZMContour_HNO3--v4-1_HNO3--v3-3_2009m02_LatPress-All-noScreen}
\caption{V3.3 (top left) and v4.1 (top right) zonal mean HNO\cs3 for
  February~2009, and differences (v3.3\,$-$\,v4.1) expressed in ppbv
  (bottom left) and percent (bottom right). \mycomment{This version has
    no screening, so that it is ``uniform'' between the versions.  I'd
    rather have a version with recommended screening for each, but
    didn't see how to easily force Ryan's program to screen v4 properly
    (not just a matter of setting Q/C thresholds since I'm still
    recommending outlier screening).}}
...
\begin{figure}
\centering\includegraphics[width=8.4cm]{figures/ComparePrecision2Panel_HNO3}
\caption{Precision of the (left) v3.3 and (right) v2.2 MLS HNO\cs3
  ...
  retrieval algorithm. \mycomment{This version of the plot has screening
    for Q/C and outliers for v3 but not v4; need to make a version with
    neither; the discussion below assumes the latter.}}
\label{fig:PrecProf}
\end{figure}
...
range of signal to noise ratios.  At some latitudes and altitudes and in
...
compare very well between 215 and 15\,hPa, above 10\,hPa the predicted
...
than 10\,hPa.  Conversely, the observed scatter at pressures higher than
100\,hPa is somewhat larger than the theoretical precision in \thisv and
much larger than the theoretical precision in v3.3.  This is related to
the spikes and increased noise in the UTLS in v3.3, and can be seen to
be much imporved in \thisv.  Nevertheless, some outliers in this region
persist in \thisv.  Procedures for screening outliers in the UTLS are
discussed below.

\subsection*{Accuracy}

The effects of various sources of
systematic uncertainty (e.g., instrumental issues, spectroscopic
uncertainty, and approximations in the retrieval formulation and
implementation) on the MLS \prevv HNO\cs3 measurements were quantified
through a comprehensive set of retrieval simulations; results for
\thisv, to be completed at a later date, are expected to be similar.
The results of the v3.3 uncertainty analysis are summarized in
Table~\ref{tab:HNO3Summary}; see \citet{SanteeEtal07hno3} for further
details of how the analysis was conducted and the magnitude of the
expected biases, additional scatter, and possible scaling errors each
source of uncertainty may introduce into the data.  In aggregate,
systematic uncertainties are estimated to induce in the HNO\cs3
measurements biases that vary with altitude between $\pm$0.5 and
$\pm$2\,ppbv and multiplicative errors of $\pm$5\,--\,15\% through most
of the stratosphere, rising to \mbox{\texttildelow$\pm$30\%} at 215\,hPa
and \texttildelow50\% at and above 2.2\,hPa.  These uncertainty
estimates are generally consistent with the results of comparisons with
correlative datasets, as discussed briefly below.
...
standard \texttt{L2GP-HNO3} files are appropriate for use in screening
at levels at and below (that is, pressures greater than) 22\,hPa.  For
those levels:
...
\item[Quality:] \qualityrule{0.8}
  ...
  147\,hPa; many such profiles are removed by setting \mycomment{By
    setting what?}

\item[Convergence:] \convergencerule{1.05}

  On a typical day this threshold for \texttt{Convergence} discards a
  very small fraction of the data, rarely eliminating even as many as
  0.5\% of the HNO\cs3 profiles.  Most such profiles show unphysical
  behavior.
...
\item[Clouds:] \screeningrule{Clouds impact HNO\cs3 data in the UTLS,
  ...
  \texttildelow10\,--\,15\% of profiles are identified in this manner,
  with the fraction of profiles possibly impacted by clouds rising to
  \texttildelow25\,--\,35\% on average in the tropics.  \mycomment{I
    haven't had a chance to check the preceding numbers.}  Clouds
  ...
  with nonzero values of \texttt{Status} be discarded because of the
  potential for cloud contamination.  While this will reject some
  ...
  it does eliminate some unphysical profiles that are not flagged by the
  outlier screening.
...
\item[Quality:] \screeningrule{Only profiles with a value of the
    \texttt{Quality} field for \texttt{HNO3-190} \emph{greater} than 0.8
    should be used in scientific study.}

  This threshold for \texttt{Quality} typically excludes
  \texttildelow1\,--\,3\% \mycomment{need to double check these numbers}
  ...
  identifying all ``bad'' ones.  Most profiles flagged by low Quality do
  not show obvious unphysical behavior.

\item[Convergence:] \screeningrule{Only profiles with a value of the
    \texttt{Convergence} field for
  ...
  \texttildelow0.5\,--\,1.5\% of the HNO\cs3 profiles.  Most of the
...
\item[Outliers:] For levels at and above (pressures less than) 4.6\,hPa,
  ...
  \texttt{Convergence} criteria defined above eliminate many of these
  profiles.
\end{description}
...
\end{tabular}
\caption{HNO\cs3 profiles on 20~February~2009 color-coded by screening.
  Cyan profiles have zero \texttt{Status}, and \texttt{Quality} less
  than 0.8 (0.5), olive-green \texttt{Convergence} greater than 1.03
  (1.4), and red both \texttt{Quality} less than 0.8 (0.5) and
  \texttt{Convergence} greater than 1.03 (1.4) for \thisv (v3.3).
  Orange profiles are those flagged by the simple screening procedure
  described above (using large negative mixing ratios at high pressures)
  after the profiles that failed \texttt{Quality} and/or
  \texttt{Convergence} tests were removed.  Black profiles are all those
  remaining (the ``good'' profiles) after the screening.  The left
  panels show all individual profiles in the day; the right panels show
  the means in each category, with the standard deviation shown as bars
  and the range as dotted lines.  The horizontal line is at 22\,hPa,
  above which HNO\cs3 is from the 190-GHz radiometer and thus not
  appropriately screened by these criteria.  The four pairs of panels
  show all \thisv profiles (top left), \thisv profiles between
  $-$20\degsym and 20\degsym\ latitude (top right), all v3.3 profiles
  (bottom left) and v3.3 profiles between $-$20\degsym and 20\degsym\
  latitude.}
