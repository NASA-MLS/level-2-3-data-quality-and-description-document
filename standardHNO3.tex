...
\thisv.  Improvement in HNO\cs3 resulted from \mycomment{whatever NJL,
  WGR, et al want to say here} (see \ref{sec:V4vsV3Overview}). Lower
\thisv values in Figure~\ref{fig:zmv3v4diffs} in the tropics at the
lowest levels result largely from these effects.
Figure~\ref{fig:zmv3v4diffs} shows an example of typical differences
between v3.3 and \thisv HNO\cs3\@.  Higher values in \thisv than in v3.3
in the tropical UTLS result primary from the reduction of adverse cloud
effects.
...
Retrievals and \texttt{Quality} and \texttt{Convergence} information
from both sets of retrievals are included in the standard HNO\cs3 files
so that the appropriate screeening values can conveniently be used at
each level.  The \texttt{Quality} and \texttt{Convergence} included in
the ``HNO\cs3'' swath are from the \mbox{240-GHz} radiometer and
...
updated for \thisv at a later date) \mycomment{not sure what the status
  of this is?}.  The overall uncertainty for an individual data point is
determined by taking the root sum square (RSS) of the precision, bias,
and scaling error terms (for averages, the single-profile precision
value is divided by the square root of the number of profiles
contributing to the average).  More details on the precision,
resolution, and accuracy of the MLS \thisv HNO\cs3 measurements are
given below.
...
\mycomment{Didn't change this except v2.2 to v3.3, which may be the
  wrong thing to do anyway.}  The effects of various sources of
...
implementation) on the MLS v3.3 HNO\cs3 measurements were quantified
...
\item[Quality:] \qualityrule{0.8}
  ...
  147\,hPa; many such profiles are removed by setting
...
\item[Clouds:] \screeningrule{Clouds impact HNO\cs3 data in the UTLS,
  ...
  unphysical profiles that are not flagged by the outlier screening.
...
\caption{HNO\cs3 profiles on 20~Feb~2009 color-coded by screening.  Cyan
  profiles have zero Status, and \texttt{Quality} less than 0.8 (0.5),
  olive-green \texttt{Convergence} greater than 1.03 (1.4), and red both
  ...
  removed.  Black profiles are all those remaining (the `good' profiles)
  after the screening.  The left panels show all individual profiles in
  the day; the right panels show the means in each category, with the
  standard deviation shown as bars and the range as dotted lines.  The
  horizontal line is at 22\,hPa, above which HNO\cs3 is from the 190-GHz
  radiometer and thus not appropriately screened by these criteria.  The
  four pairs of panels show all \thisv profiles (top left), \thisv
  profiles between $-$20 and 20\degsym\ latitude (top right), all v3.3
  profiles (bottom left) and v3.3 profiles between $-$20 and 20\degsym\
  latitude.}
...
\item No idea what to put here.  Perhaps something about 147/100\,hPa
  notch, but that is actually much improved when above screening is
  applied (as opposed to in testing when I was comparing unscreened
  quantities).  
\end{itemize}
