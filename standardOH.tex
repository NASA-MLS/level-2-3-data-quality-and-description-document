...
is generally similar between v2.2 and v3.x, there were significant
improvements in v4.x inherited to \thisv\CheckThis. Previous to v3.x, OH data near the mesospheric density peak,
\mytexttilde0.032 hPa, often show considerable amount of data flagged with negative precision,
...
sometimes causes gaps in zonal mean time series. Starting with v4.x, the software
...
notice more zig-zags in nighttime mesospheric OH vertical
...
The estimated uncertainties, precisions, and resolution for \thisv\CheckThis OH are
...
A typical OH profile and the associated precisions (for both \prevv\CheckThis and
\thisv\CheckThis) are shown in Figure~\ref{fig:OHprofiles}. The profile is shown in
...
density profile shows two peaks at \mytexttilde45\,km and \mytexttilde75\,km. The
night OH profile exhibits the narrow layer at \mytexttilde82 km
...
than 10\% relative precision with one day of data (\mytexttilde100 samples)
...
\centering\includegraphics[width=0.7\linewidth]{figures/avk-OH}
  \caption{Typical two-dimensional (vertical and horizontal along-track)
    averaging kernels for the MLS \thisv\CheckThis OH data at 35\degsym N for
...
\caption{Zonal mean of retrieved OH and its estimated precision
  (horizontal error bars) for October 1, 2009 averaged over 29\degsym
  N to 39\degsym N\@. The average includes  \mytexttilde100 profiles. Panel (a) shows
  \thisv\CheckThis OH vmr vs. pressure for day (black) and night (blue). The 
  retrieved night OH concentration is near zero for altitudes below 1\,hPa.  Note that
  the day-night difference is required for altitudes below 10\,hPa. Panel (c)
  shows the same data in (a) converted into density units. Panels (b) and (d)
  are equivalent to (a) and (c) but using \prevv\CheckThis OH  data.}
...
and/or design requirements. Biases can be eliminated by taking day-night differences from
32--15\,hPa. For 10--0.1\,hPa, the observed night OH
...
\evenstatusrule
\item[Quality:] \screeningrule{MLS \thisv\CheckThis OH data can be used
...
\citet{WangEtal:2008}.  The comparison among v2.2 and \thisv\CheckThis show
no significant differences except for the mesosphere where \thisv\CheckThis OH
