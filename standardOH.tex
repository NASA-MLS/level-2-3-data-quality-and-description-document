...
\label{sec:StandardOH}

\subsection*{Introduction}

The THz radiometer is dedicated to measuring OH in the 2.5\,THz spectral
region.  A description of OH data quality, precision and systematic
errors for an earlier version, v2.2, is given in \citet{PickettEtal08}.
The validation studies are described in \citet{PickettEtal08} and
\citet{WangEtal08}. An early validation using v1.5 software is also
described in \citet{PickettEtal06}. While there are significant
improvements from v1.5 to v2.2, the OH data quality in \thisv is generally
similar to v2.2. One noticeable difference is the larger nighttime
offset below 10\,hPa in \thisv. This offset can be removed with the
day-night difference, which is recommended for altitudes below 10\,hPa.
...
uncertainties are from v2.2, but are not expected to change significantly
in \thisv.
...
for high and low latitudes are very similar to Figure~\ref{fig:AvkOH} for
most pressure levels.  At the topmost two pressure levels, 0.0046\,hPa
and 0.0032\,hPa, the vertical resolution is slightly better at the
equator than at 70\degsym N.

\subsection*{Precision}

A typical OH profile and the associated precisions (for both v2.2 and
...
\begin{figure}
  \centering\dontincludegraphics[width=0.7\linewidth]{figures/avk-OH_day_night_35N}
...
\begin{figure}
\begin{tabular}{@{}l@{}l@{}}
\dontincludegraphics[width=0.5\linewidth]{figures/plot-OH-a}&
\dontincludegraphics[width=0.5\linewidth]{figures/plot-OH-b}\\
\dontincludegraphics[width=0.5\linewidth]{figures/plot-OH-c}&
\dontincludegraphics[width=0.5\linewidth]{figures/plot-OH-d}\\
\dontincludegraphics[width=0.5\linewidth]{figures/plot-OH-e}&
\dontincludegraphics[width=0.5\linewidth]{figures/plot-OH-f}%
\end{tabular}
\caption{Zonal mean of retrieved OH and its estimated precision
  ...
  the day-night difference is required for altitudes below 10\,hPa.
  Panels (e) and (f) are equivalent to (c) and (d) but using v2.2 OH
  data.}
...
uncertainty. These accuracy calculations are for v2.2 products. While no
significant change is expected from v2.2 to \thisv, a comprehensive error
analysis for \thisv will be conducted.  Bias uncertainty can be eliminated
by taking day-night differences from 32\,--\,14\,hPa. For
10\,--\,0.1\,hPa, the observed night OH concentration is small and
...
These relock events occur when the tuning range of the laser is less
than the thermal excursion over an orbit and over a day. This thermal
effect depends on the albedo of the Earth as seen by the GLLO radiator.
In these cases the \texttt{Status} flag is set to 257 and the profile is
ignored. This can present a problem when compiling maps, because the
missing data may appear at the same latitude and longitude on successive
days.

\subsection*{Data for 2009 and beyond under investigation}

As discussed in section~\ref{sec:OHNote}, we have performed the 30-day OH
measurement during 2011 and 2012 and plan to continue it in the coming
years as the solar maximum approaches (expected to be in
2013\,--\,2015). However, due to the aging of the instrument and the
rather long suspension period during 2009\,--\,2011, ``new'' OH data
acquired since 2011 are significantly noisier than the previous years
and likely require additional calibration.  We have been examining the
data carefully.  The release of the ``new'' OH data for scientific use
will depend on the results of this continuing effort.  

Scientists interested in these observations are advised to consult the
MLS science team.
...
\citet{WangEtal08}. The comparison between v2.2 and \thisv show no
