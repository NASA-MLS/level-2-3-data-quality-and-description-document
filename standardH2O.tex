...
The main H\cs2O differences between \thisv and \prevv relate to
...
\prevv. An improved cloud detection methodology has been developed
...
\end{center}
\caption{A comparison of \prevv (blue) to v2.2 (green) and \thisv
...
Figure~\ref{fig:avgmlsprflcomp} compares MLS \thisv to \prevv and
...
for \thisv and \prevv are nearly identical except for pressures
...
\end{center}
\caption{A comparison of \thisv (red) to \prevv (blue) and v2.2 (green) water
...
same approach has been used for \thisv) in \citet{ReadEtal:2007} and
\citet{LambertEtal:2007}. For pressures between 316\,--\,178\,hPa,
...
Please note the discussion of a potential drift in the MLS water vapor
...
\caption{Mapped fields from AIRS v6 (left), MLS v2.2 (center-left), MLS
  \prevv (center-right), and MLS \thisv (right) pressures between
...
\caption{Mapped fields from AIRS v6 (left), MLS v2.2 (center-left), MLS
  ...
  measurement is mostly suitable for H\cs2O greater than 10\,ppmv.}
\label{fig:airsmlsjja2005maps}a
\end{figure}

Apart from the differences noted above, the MLS \thisv H\cs2O is similar
to the MLS \prevv and MLS v2.2 products, the latter described and
...
information. MLS \thisv is a part of the second SPARC Water Vapor
Assessment (WAVAS-II) activity.

\subsection{Potential drift in lower stratospheric water vapor}
\label{sec:H2ODrift}

\citet{HurstEtal:2016} discuss how comparisons between MLS lower
...
(roughly 0.6 to 1.5\% per year), starting around 2009.  Comparisons with
the ACE-FTS instrument, and with measurements of upper stratospheric
water vapor from ground-based microwave sensors, provide evidence for a
smaller (\texttildelow+0.2\% per year) drift.  Investigations by the
MLS team have indicated that this latter, smaller, drift is consistent
with observed changes in instrument performance (correction of which is
a goal for future versions of the MLS data processing
software). However, these observed instrument changes cannot (at least
in our current understanding) account for the larger drift reported by
\citeauthor{HurstEtal:2016}. These issues are under active investigation by
the MLS team.
...
\item[Quality field:] \screeningrule{Only profiles with a value of the
    ...
    scientific studies.}  Note that this is a change from the value
  originally recommended for \thisv, as described at the end of this
  subsubsection.  Also, note the ``additional screening'' information
  given below.
\item[Convergence field:] \screeningrule{Only profiles with a value of
    ...
    scientific studies.}
\item[Additional screening to avoid outliers:] Even after application of
  ...
  and stratosphere.  These should be excluded from analyses by simply
  neglecting any H\cs2O profile where the mixing ratio at any pressure
  at or larger than 1\,hPa (i.e., altitudes below that pressure level)
  is less than 0.101\,ppmv.  \textbf{Note that this additional screening
    is an additional step beyond those originally recommended for
    \thisv\ H\cs2O.}
  
\end{description}

\subsubsection{Updated \texttt{Quality} screening recommendation}
\label{sec:H2OScreeningChange} A byproduct of the aging in MLS that
gives rise to the drift in the water vapor product and 190-GHz N\cs2O
(see section~\ref{sec:StandardN2O}) is that the \texttt{Quality} metric
for the H\cs2O product exhibits a decreasing trend (most notable in the
years following \texttildelow2009).  This decrease is not correlated
with any actual decline in the accuracy of the MLS H\cs2O product, which
appears largely unchanged (drift issues aside).  However, application of
the previously recommended 1.45 \texttt{Quality} threshold leads to
rejection of an unreasonably large fraction of the H\cs2O profiles in
the more recent years of the Aura mission, particularly during southern
winter.  The revised 0.7 \texttt{Quality} threshold restores these
profiles without erroneously identifying any obviously poor profiles as
acceptable.  Figure~\ref{fig:H2OScreeningChange} compares the amounts of
H\cs2O observations flagged as acceptable according to the ``old'' and
``new'' screening methods over the life of the Aura mission to date.
The new screening method rejects fewer H\cs2O profiles than the old one.
In addition the old method rejects more profiles in the later part of
the mission than in the earlier years, a behavior the new method avoids.

\begin{figure}[htb]
  \includegraphics[width=\textwidth]{figures/quality_yields_h2o}
  \caption{A time series of daily good data yields using the previous
    data quality screening for v04.2x (grey) and the current revised
    quality and profile value screening method (black).}
  \label{fig:H2OScreeningChange}
\end{figure}
...
Note the discussion of the drift in MLS stratospheric water vapor above.
