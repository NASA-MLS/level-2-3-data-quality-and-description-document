...
retrieval. The vertical grid for H\cs2O is: 12 levels per decade change
in pressure (LPD) for 1000\,--\,1 hPa, 6\,LPD for 1.0\,--\,0.1\,hPa, and
3\,LPD for 0.1\,--\,10\cp{$-$5}\,hPa. The horizontal grid is every
1.5\degsym along the orbit track.  H\cs2O is unusual among MLS products
in that it is assumed that the logarithm of the mixing ratio, and not
mixing ratio itself, varies linearly with log pressure; although,
H\cs2O, like the other MLS constituents is retrieved in VMR (not $\log$
VMR). Vertical and horizontal interpolations of H\cs2O should be
performed in $\log(\text{VMR})$ space.
...
relative to \prevv. The InitUTH phase retrieves H\cs2O on a more coarse
6-LPD grid. The InitUTH H\cs2O is used for the initial guess for the
final 12-LPD retrieval and sets smoothing constraints for the profile
shape instead of smoothing to the shape of a climatological \emph{a
  priori} profile.  The \texttt{InitUTH} phase has been expanded to use
...
to have its top moved from 100\,hPa (in \prevv) to 10\,hPa (in \thisv),
for a more seamless transition from the troposphere to the
...
\thisv H\cs2O data. For pressures $\geq100\,\text{hPa}$, the precisions given are
