...
and resolution for \thisv\CheckThis HO\cs2 are summarized below in
...
A typical HO\cs2 profile and the associated precisions (for both \prevv\CheckThis
and \thisv\CheckThis) are shown in Figure~\ref{fig:HO2profiles}. The profile is
...
\mytexttilde82\,km \citep{PickettEtal:2006:NightOH}, which is not shown in
Figure~\ref{fig:HO2profiles} since MLS HO\cs2 data is not recommended
for altitudes above 0.046\,hPa (\mytexttilde70\,km). Precisions are
...
data (\mytexttilde2000 samples) for most pressure levels over
22--0.046\,hPa.
...
\caption{Monthly zonal mean of retrieved HO\cs2 and its estimated
  ...
  29\degsym N to 39\degsym N. Panel (a) shows \thisv\CheckThis HO\cs2 vmr vs.\
  ...
  (c). Panels (e) and (f) are equivalent to (c) and (d) but using \prevv\CheckThis
  data.}
...
\item[Quality:] \screeningrule{MLS \thisv\CheckThis HO\cs2 data can be used
...
\item[Convergence:] \convergencerule{1.1} In version v2.2 this
  ...
  version, \thisv\CheckThis, there are often zero or very few non-converged
  profiles.
\end{description}
...
given in \citet{PickettEtal:2006:Validation}. Differences between v2.2 and \thisv\CheckThis show
