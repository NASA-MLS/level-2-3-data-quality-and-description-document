...
volume sampled. This volume has a vertical extent of \mytexttilde3\,km, with
\mytexttilde300\,km and 7\,km along and cross track respectively.
...
\item[Pressure range (215--83\,hPa):] \standardrangestatement\ The
  maximum detectable IWC is \mytexttilde100\,mg/m\cp3.
...
At wintertime mid-to-high latitudes, strong stratospheric gravity
waves may induce large fluctuations in the retrieved tangent pressure,
and cause false cloud detection with the ``2$\sigma$, 3$\sigma$''
screening method. The false cloud detection seems to affect the
100\,hPa pressure level most, as expected for such impact coming from
the lower stratosphere.

\subsection{Comparisons with other datasets}
  
Scatter plots of \thisv\CheckThis vs v2.2 IWC show mean biases of less than 10\%
in the pressure range 215--83\,hPa and histograms show that
the random noise in \thisv\CheckThis IWC is generally larger than in v2.2 (see
...
retrieval to the changes in \thisv\CheckThis UTLS H\cs2O.  These effects are largest at
...
the MLS \thisv\CheckThis IWC is similar to the MLS v2.2 product described and
...
that MLS cloud tops are slightly higher by \mytexttilde1\,km than the
...
\begin{figure}
  ...
  \caption{MLS v5, v4 and v2 IWC comparisons for July 2009 at 146\,hPa
    ...
    Gaussian error function.  (b) Right: Scatter plots of IWC v5 vs.\
    v2 (black points) with dashed red lines indicating the 1:1 line,
    dashed yellow lines the $1\sigma$ uncertainties. The solid yellow
    lines are linear fits to the data (which lie practically on the
    1:1 line).}
\label{fig:IWC}
\end{figure}
