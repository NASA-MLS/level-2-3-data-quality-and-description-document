...
The MLS \thisv temperature product is nearly identical to the \prevv product and is generally 
similar to the v3.3 product
and to the v2.2 product that is described in~\citet{SchwartzEtal:2008:Validation}.
...
Mean differences between \thisv temperature and \prevv temperature
are typically less than 0.2\,K, although vertically oscillating bias 
...
teams'' (FPIT)
%https://gmao.gsfc.nasa.gov/products/documents/GEOS_5_FPIT_File_Specification_ON02_v1_2.pdf
temperatures as their retrieval \emph{a priori}, each using the
version available at the time of processing.  \prevv uses GEOS-5.91 from the beginning
of the mission through 2016 and GEOS-5.124 from 2017 into 2020,
whereas \thisv uses GEOS-5.124 from the beginning of the mission into
2020, and differences in these \emph{a priori} can result in small differences in retrieved values.
...
and tangent-point pressure. The standard \thisv and\prevv temperature products are
both taken from the \texttt{CorePlusR3} phase, which adds radiances from the
...
\caption{Temperature differences between \thisv and \prevv standard
  ...
  unscreened (red).  2009 data are shown.  Top row is mean difference
  ...
  middle row of panels shows that there is considerably more scatter in unscreened data at
  the bottom of the retrieval, particularly at low latitudes.  The bottom row's 
  difference between red and green lines is 
...
perturbations in MLS retrieved quantities, including temperature.  Indeed, \emph{a priori  }
knowledge of temperature from analysis is often so good in the lower
and middle atmosphere that perturbations of a few percent (a level typically considered to be
...
``significant'' for temperature.  In the troposphere where \emph{a
  priori} knowledge may be on the order of 1--2\,K, temperatures from an analysis 
like GEOS-5 (the retrieval \emph{a priori}) may be more reliable than the 
MLS retrieval product, and   
differences between the MLS and GEOS-5 temperatures can be useful in the identification of
retrieval artifacts.   
Cloud-induced perturbations generally occur in the
tropics and mid-latitudes (where convective storms occur) in both the \thisv
and \prevv retrieval versions. In unscreened data at retrieval levels
316\,--\,178\,hPa, clouds produce primarily positive outliers in \thisv and \prevv.  The top panels of
Figure\,\ref{fig:v5v4__215hPa_pcolorhist2} show this behavior at
215\,hPa. Conversely, at 121\,hPa, outliers are primarily positive in \thisv and \prevv.
...
The bottom panels show histograms of 
  differences of the screened proon a finer vertical scale.  Outliers beyond $\pm$20\,K
...
(20\degsym\, S--20\degsym\,N) \thisv and \prevv temperature from 2009 are shown in
Figure\,\ref{fig:v5v4_68-316hPa_linehists}.  Tails of low, unscreened
outliers are particularly evident from 316--147\,hPa,
but are greatly reduced by recommended screening (magenta).  



\begin{figure}
  \centering\includegraphics[angle=0,width=\linewidth]{figures/v5v4_68-316hPa_linehists}
  \caption{Panels show histograms of tropical (20\degsym S--20\degsym N)
    ...
    and screened v04.2x, respectively. Red and magenta are unscreened
    and screened v05.00, respectively.  Distribution means and standard
...
this document is 0.001\,hPa, but
temperature is retrieved, in both \prevv and \thisv, up to the 
0.0001\,hPa level. The 0.00046\,hPa level is typically near the top tangent 
point of the MLS scan while the 0.00022\,hPa surface is often above
the height of the highest tangent point in the MLS limb scan (depending 
upon latitude). The estimated precision of the retrieval 
at both of these levels is less than half of the precision of the retrieval 
\emph{a priori}.  However, as can be seen from Figure~\ref{fig:AvkTemperature_HR} 
for both latitudes shown, 
the averaging kernels of the 0.00046\,hPa retrieval level is sharply peaked at 
0.00046\,hPa, while the averaging kernel of the 0.00022\,hPa retrieval level shows a larger contribution 
from 0.00046\,hPa than from 0.00022\,hPa.
As was the case in \prevv, the precisions 
of the 0.00046\,hPa and 0.00022\,hPa levels in \thisv are set negative
``by hand'' to advise against their casual use,
but for \thisv, we are recommending that 0.00046~hPa temperature may be generally useful, 
while 0.00022\,hPa may have some value in for some scientific studies.  Further validation of 
\thisv temperature at these levels is planned. 
...
Table\,\ref{tab:TemperatureSummary}.  Vertical resolution, shown on the
left panel, is 3\,--\,4\,km from 261\,hPa to 10\,hPa, degrades to 7\,--\,8\,km at 1\,--\, 0.1\,hPa, and to 11\,km at 0.01\,hPa, and to 12\,km at 0.0001\,--\,0.0002\,hPa.  Along
track resolution is \texttildelow165\,km from 261\,hPa to 0.1\,hPa and
degrades to 280\,km at 0.001\,hPa and to 316\,km at 0.00046\,hPa.  The cross-track resolution is set by
...
mesosphere and 4\,K at 0.00046\,hPa, are given, for selected levels, in column\,2.  Column\,3 gives
...
should be nearly identical to that of \prevv. This accuracy was
estimated both by modeling the impact of uncertainties in measurement
...
of parameter uncertainties, as discussed in~\citet{SchwartzEtal:2008:Validation}.
...
\item[Cloud Screening:] The ``low cloud'' bit of the temperature
  ...
  so it cannot be used to screen temperature as it was in 
  version v03.2.  However, cloud impacts can largely be removed using the ice
  ...
  outliers seen in Figure~\ref{fig:v5v4_68-316hPa_linehists}.  This
...
discussion of artifacts may be found in~\citet{SchwartzEtal:2008:Validation}.
...
\caption{The left panel shows globally-averaged mean differences between
  MLS \prevv temperature and eight correlative data sets.  Criteria for
  coincidences are described in detain in~\citet{SchwartzEtal:2008:Validation}.  The
...
\caption{Zonal mean of the difference between MLS \thisv temperature and
  GEOS-5.124 temperature (upper), and variability
  about that mean (lower), averaged for 2009.}
\label{fig:MLSminusGEOS5_zonalmean}
\end{figure}

\sloppypar\citet{SchwartzEtal:2008:Validation} describes detailed comparisons of MLS v2.2
temperature with products from the Goddard Earth Observing System,
version~5~\citep{RieneckerEtal:2007} (GEOS-5), the European Center for
...
Radiometry (SABER)~\citep{MlynczakAndRussell:1995}, the Halogen Occultation
Experiment~\citep{HervigEtal:1996} (HALOE) and the Atmospheric Chemistry
Experiment~\citep{BernathEtal:2004} (ACE), as well as to radiosondes from
