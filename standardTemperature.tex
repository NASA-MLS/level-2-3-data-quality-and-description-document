...
Table~\ref{tab:TSummary} summarizes the measurement precision,
resolution, and modeled and observed biases.  The following sections
provide details.

\subsection*{Differences between \thisv and v3 }
Mean differences between \thisv temperature and v3.3 temperature are
typically less than 0.5\,K and exceed 1\,K only in the tropical
troposphere.  In the tropics, a cold bias in unscreened \thisv relative
to v3.3 reaches 2\,K, and is reduced to 1.5\,K by the recommended
screening, discussed below.  The MLS retrieval algorithm is divided into
``phases'' that retrieve a subset of the products from a subset of the
radiances.  Versions v3.x and v2.x took the standard temperature product
from FinalPtan, a preliminary phase using only the O$_2$ bands and
focused on retrieving only temperature and tangent-point pressure. The
MLS \thisv temperature product is taken from the CorePlusR3 phase, which
adds radiances from the 240-GHz radiometer.  This temperature and
pointing is used for all subsequent constituent retrievals, so \thisv
temperature, pointing and constituent retrieval algorithms are more
internally consistent with other retrieved products than were those of
previous versions.  While in other respects \thisv retrieval algorithms
are similar to those of v3.x and v2.2, biases with respect to GEOS-5 are
somewhat different.  The tropospheric and stratospheric \emph{a priori}
temperature profiles used in \thisv are from GEOS-5.9, and are quite
similar to the GEOS-5.7 and GEOS-5.2 profiles used in previous version,
departing most significantly near the stratopause.
...
propagation of radiometric noise and \emph{a~priori} uncertainties
through the measurement system.  These values, which range from 0.6\,K
in the lower stratosphere to 2.5\,K in the mesosphere, are given, for
selected levels, in column~2.  Column~3 gives the rms of differences of
values from successive orbits (divided by the square-root of two as we
are looking at the difference of two noisy signals) for latitudes and
seasons where longitudinal variability is small and/or is a function
only of local solar time.  The smallest values found, which are for
high-latitude summer, are taken to be those least impacted by
...
average, 12\% higher than other tropical values and the occurrence rates of high
outliers with mixing ratios greater than 150\,ppbv is about twice that
of the rest of the tropical ensemble.  At 147\,hPa and 100\,hPa the
rejected profiles are 

\item[Convergence:] \convergencerule{1.021}
  This  \texttt{Convergence} criterion rejects 0.12\% of profiles.
Profiles that fail to meet the convergence criterion of 1.02 are only very slightly noisier than those that converge more fully and they are only 0.12\% of profiles, so this screening criterion is only of very limited utility and \texttt{Convergence} can generally be ignored without peril.  
Almost all of the retrieval in the retrieval phase that produces \thisv CO 
converge to their target.

\item[Precision:]
\thisv temperature does not have the ``low cloud'' flag that was used
to screen previous versions, the L2gpPrecision field can be used to
further eliminate outliers that are believed to be the result of thick
clouds, primarily in the tropics.  If careful screening of the
troposphere is required, levels 261--147\,hPa should be avoided if any
of the following criteria are met: 
\begin{itemize}
\item 316-hPa L2gpPrecision > 1.1\,K AND Latitude > -60$^\circ$ 
\item 261-hPa L2gpPrecision > 0.7\,K 
\item 215-hPa L2gpPrecision > 0.825\,K 
\end{itemize}
\end{description}
...
\begin{figure}
\centering\includegraphics[width=0.7\linewidth]{figures/MLSv33TminusGEOS5}
