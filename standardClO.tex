...
As in previous versions, in \thisv\CheckThis the standard ClO product is derived
...
over much of the profile in \prevv\CheckThis, and the \thisv\CheckThis retrieval does not
depart dramatically from \prevv\CheckThis, although the biases present at the
lowest retrieval levels have been further reduced (see
Section~\ref{sec:cloBias}).  The MLS \thisv\CheckThis ClO data are
...
systematic uncertainty of the \thisv\CheckThis ClO measurements as a function of
...
of the \thisv\CheckThis ClO data as determined from the full width at half
maximum of the rows of the averaging kernel matrix shown in
Figure~\ref{fig:AvkClO} is \mytexttilde3--4.5\,km throughout the
retrieval range (with a mean of \mytexttilde3.5\,km).  The averaging
kernels are sharply peaked at all levels, even at 147\,hPa, where the
kernel indicates that the retrieval has sensitivity to ClO abundances
higher in the atmosphere (particularly 100\,hPa).  Nevertheless, the
147\,hPa surface provides independent information.
...
\caption{Precision of the (left) \thisv\CheckThis and (right) \prevv\CheckThis MLS ClO
...
data is virtually unchanged in \thisv\CheckThis, ranging from
...
the retrieval formulation and implementation) on the MLS \thisv\CheckThis ClO
...
MLS \thisv\CheckThis ClO data (see Table~\ref{tab:ClOSummary}).  These values
for the overall uncertainty in \thisv\CheckThis ClO, which are slightly larger
(poorer accuracy) at 100 and 147\,hPa than those estimated for the
\prevv\CheckThis data \citep[see][]{EOSMLSV4Quality}, do not take into account
...
present in earlier MLS data versions has been mitigated in \thisv\CheckThis.
Some of the differences between \thisv\CheckThis and \prevv\CheckThis ClO can thus be
attributed to the improvement in H\cs2O between the two versions.
Zonal-mean differences from \prevv\CheckThis are generally small (less than
...
depicts \thisv\CheckThis and \prevv\CheckThis nighttime mixing ratios for a representative
...
has been greatly reduced in \thisv\CheckThis; thus the two ClO products
correspond more closely now than they did in \prevv\CheckThis.  Nevertheless,
...
\caption{Zonal mean pressure-latitude cross sections of MLS daytime
  (ascending) ClO, for (left) \thisv\CheckThis, (middle) \prevv\CheckThis, and (right)
  their differences (\thisv\CheckThis minus \prevv\CheckThis, in percent).  No bias
...
\begin{figure}
 ...
 \includegraphics[width=0.48\linewidth]{figures/clov5v4bylat_ClO-640_2009d258}%
   \end{center}
\caption{Nighttime \thisv\CheckThis (dark blue) and \prevv\CheckThis (light blue) MLS ClO
...
\caption{MLS \thisv\CheckThis (dark colors) and \prevv\CheckThis (light colors) profiles
  ...
  panels, comparing \thisv\CheckThis and \prevv\CheckThis \texttt{ClO-640} retrievals,
  \thisv\CheckThis and \prevv\CheckThis \texttt{ClO-190} retrievals, \prevv\CheckThis
  \texttt{ClO-640} and \texttt{ClO-190} retrievals, and \thisv\CheckThis
...
retrieval pressure levels, where substantial biases present in \prevv\CheckThis
have been ameliorated in \thisv\CheckThis.  To investigate in more detail the
magnitude of and temporal variations in the bias in the \thisv\CheckThis MLS ClO
data and compare them to \prevv\CheckThis, we show in
...
147--10\,hPa.  In both figures, bold colors represent \thisv\CheckThis data,
while corresponding pale colors represent \prevv\CheckThis data.  To guide the
...
pressure.  At 46\,hPa, a negligible negative bias present in \prevv\CheckThis is
further reduced in \thisv\CheckThis.  At 68\,hPa, the small negative bias in
\prevv\CheckThis is considerably lessened but is still evident in \thisv\CheckThis.  At
...
global-mean bias decreased by about half between \prevv\CheckThis and \thisv\CheckThis.
...
strong latitude dependence at this level in \prevv\CheckThis has been somewhat
flattened in \thisv\CheckThis.  However, unlike at 68 and 100\,hPa, the global
mean bias has increased slightly at 147\,hPa in \thisv\CheckThis.  In contrast
...
\caption{Estimates of the bias in MLS \thisv\CheckThis ClO data in
  ...
  shows monthly zonal means of \thisv\CheckThis MLS nighttime (solar zenith
  angle $>$ 100{\degsym}) ClO measurements averaged over a
  \mbox{16-yr} period (2005--2020, filled circles). Results for \thisv\CheckThis
  are depicted in bold colors, those for \prevv\CheckThis in corresponding pale
...
with season) have so far met with little success.  Thus the bias is
...
estimates and the \thisv\CheckThis ClO data themselves; see
...
grid, must be interpolated to the MLS Level~2 measurement locations
being considered and subtracted from the ClO mixing ratios (daytime or
nighttime) at those locations.  The bias correction must be performed
\emph{before} interpolation to a different vertical coordinate (e.g.,
potential temperature). An ASCII file containing the estimated ClO
bias values has been embedded in the PDF version of this document;
...
%\centering\includegraphics[width=\linewidth]{figures/plotBiasV5-Clim_2020_ClO640}
   \caption{Estimates of the bias in MLS \thisv\CheckThis nighttime (solar
     ...
     (2005--2020).  Results for \thisv\CheckThis are depicted in bold colors,
     those for \prevv\CheckThis in corresponding pale colors.  The horizontal
...
were reported by \citet{EOSMLSV33V34Quality}.  Comparisons of \thisv\CheckThis
...
\item[Convergence:] \convergencerule{1.05} 
  ...
  from \prevv\CheckThis) discards very few (0.5\% or less) of the ClO profiles,
...
\begin{itemize}
\item As discussed in Section~\ref{sec:cloacc}, non-negligible biases
  are present in both daytime and nighttime \thisv\CheckThis ClO mixing ratios
  at and below (i.e., pressures larger than) 68\,hPa.  The bias should
  be corrected by subtracting from the individual measurements at each
  affected retrieval level the altitude- and latitude-dependent bias
  estimates reported in an ASCII file that has been embedded in the
  PDF version of this document; instructions for extracting this file
  are given in Appendix~\ref{appsec:ClO}.  This file is also available
  at
  \url{https://mls.jpl.nasa.gov/data/MLS-Aura_ClO-BiasCorrection_v05.txt}.
