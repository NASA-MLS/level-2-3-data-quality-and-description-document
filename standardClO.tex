...
\citep{LiveseyEtal13qualdoc}; in particular, the substantial
(\textasciitilde0.1--0.4\,ppbv) negative bias present in the v2.2 ClO
...
radiometer, but these data have poorer precision.)  The MLS \thisv ClO
...
resolution of the measurements.  Consequently, the vertical resolution
...
Figure~\ref{fig:AvkClO}, is \textasciitilde3--4.5\,km (with a mean of
\textasciitilde3.5\,km).  As in \prevv, in \thisv the averaging
kernels are sharply peaked at all levels, including 147\,hPa.  Thus,
although some degree of overlap is present, the 147\,hPa surface
provides independent information in \thisv.  Figure~\ref{fig:AvkClO}
also shows horizontal averaging kernels, from which the along-track
horizontal resolution is determined to be \textasciitilde250--500\,km
over most of the vertical range.  The cross-track resolution, set by
the width of the field of view of the \mbox{640-GHz} radiometer, is
\textasciitilde3\,km.  The along-track separation between adjacent
retrieved profiles is 1.5{\degsym} great circle angle
(\textasciitilde165\,km), whereas the longitudinal separation of MLS
...
\textasciitilde0.1\,ppbv over the interval 100 -- 3\,hPa to
\textasciitilde0.3\,ppbv at 1\,hPa and 147\,hPa.  The smoothing of the
...
in some cases be even smaller \citep{TooheyvonClarmann13}).
...
biases of \mbox{\textasciitilde$\pm$0.1\,ppbv} from 100 to 32\,hPa and
less than $\pm$0.05\,ppbv above 22\,hPa and multiplicative errors of
\mbox{\textasciitilde$\pm$5--20\%} throughout the stratosphere.
...
\begin{figure}
\begin{center}
\includegraphics[width=0.3\linewidth]{figures/InspectZMContour_ClO--v4-1_ClO--v3-3_2009m04_LatPress-All-noScreen-panel1}
\includegraphics[width=0.3\linewidth]{figures/InspectZMContour_ClO--v4-1_ClO--v3-3_2009m04_LatPress-All-noScreen-panel2}
\includegraphics[width=0.3\linewidth]{figures/InspectZMContour_ClO--v4-1_ClO--v3-3_2009m04_LatPress-All-noScreen-panel3}
\end{center}
\caption{Zonal averages of MLS ClO profiles from the month of April
  2009, showing the MLS \thisv mixing ratios (left), the \prevv mixing
  ratios (middle), and their differences in ppbv (\prevv minus \thisv,
  right) as a function of pressure.  The month shown represents
  ...
  polar lower stratosphere in either hemisphere.}
...
Hemisphere early autumn gives very similar results. This figure shows
that a small, somewhat latitude-dependent, negative bias is still
evident at 68 and 100\,hPa in \thisv, of approximately the same
magnitude as in \prevv at 68\,hPa but a little larger at 100\,hPa,
especially in the Northern Hemisphere.  At 147\,hPa, \prevv displays a
strongly latitudinally varying bias that is positive at middle and low
latitudes and slightly negative in the polar regions (at least on some
days).  In \thisv, ClO mixing ratios at 147\,hPa are slightly lower
than they were in \prevv, and consequently the low-latitude positive
bias has been reduced, although it remains substantial (exceeding
0.7\,ppbv in some cases), and the high-latitude negative bias has been
exacerbated.
...
conditions of continuous daylight or continuous darkness in the summer
or winter at high latitudes, however.  Moreover, under certain
circumstances inside the winter polar vortices, chlorine activation
leads to nonnegligible ClO abundances even at night.  In this case,
...
measurements at each affected retrieval level.

\begin{figure}
\centering\includegraphics[width=15.0cm]{figures/plotBiasV4ByMonth}
...
To investigate the magnitude of the bias in the \thisv MLS ClO data
and the temporal variations in it, we show in
Figure~\ref{fig:cloBiasByMonth} monthly zonal means of MLS nighttime
ClO measurements from 2009 for pressure levels 147--68\,hPa.  For each
...
147--10\,hPa.  To guide the eye, the global mean value of the bias
from 2009 is indicated for each level (colored solid lines) in both
figures.  In Figure~\ref{fig:cloBiasByMonth}, the corresponding \prevv
data are also shown for comparison.  As discussed above, the
magnitude, and at 147\,hPa even the sign, of the bias varies with
latitude, and Figures~\ref{fig:cloBiasByMonth} and
\ref{fig:cloBiasByPress} make clear that application of a constant
bias correction for all latitudes is not appropriate.  However,
although Figure~\ref{fig:cloBiasByMonth} reveals significant
month-to-month variability, for most studies a time-invariant
latitudinally varying bias correction is adequate.  An ASCII file
containing the altitude- and latitude-dependent \thisv ClO bias
correction values \mycomment{will be made} available from the MLS web
site \mycomment{once more data from the mission have been reprocessed
  to \thisv}.
...
\citet{LiveseyEtal13qualdoc}.  Repeating similar analyses for \thisv
...
\item Significant biases are present in both daytime and nighttime
  ...
  altitude- and latitude-dependent bias estimates given in the ASCII
  file \mycomment{to be made} available from the MLS web site.
\end{itemize}
...
Latitude-dependent bias\footnote{Correct for the bias by subtracting
  ...
  \mycomment{to be made} available from the MLS web
  site.}\gdef\fnDirect{\value{mpfootnote}} \\
%
