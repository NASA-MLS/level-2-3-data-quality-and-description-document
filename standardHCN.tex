...
No changes specific to the HCN retrieval were made between \vii and \viii. For \viv, a new phase was introduced in which HCN is retrieved using only Bands 6 and 27, rather than all of R2; this phase is also used in \thisv. The \viv HCN had obvious biases in the lower stratosphere, but they were far less severe than in \vii and \viii, both of which had large regions with negative mixing ratios. In \prevv and \thisv the biases appear to be reduced further; the changes between \prevv and \thisv are small. Between 10\,hPa and 3\,hPa, \prevv and \thisv HCN are somewhat lower than all prior versions. The lowest recommended level for \thisv is currently 21\,hPa, as it was for \prevv and \viv. However, it is possible that further validation work could show that the \thisv HCN data are suitable for scientific use between 31\,hPa and 68\,hPa.
...
\caption{\label{fig:HCNvsACE} Time series of HCN at selected latitudes and pressure levels from MLS versions \viv, \prevv, and \thisv, with ACE-FTS version~5.3 for comparison. The MLS data are plotted using translucent colors; purple areas are where \thisv (magenta) and \prevv (cyan) are overlapping.  }
\end{figure}
...
\begin{description}
  \item[Pressure range:] \screeningrule{21--0.1\,hPa} \standardrangestatement\ Data in the 31\,hPa to 68\,hPa range may prove to be usable after further validation. \standardprecisionrule \evenstatusrule
...
Specific achievable improvements in the HCN retrieval remain to be determined.
...
% The precision and scatter are essentially identical in all of versions 4, 5 and 6, so this table is not changed from earlier versions of this document
  ...
  1--0.1      & 12 $\times$ 500 & 50   & 50        & \\
  21--1       & 10 $\times$ 300 & 30   & 50        & \\
  68--21      & 10 $\times$ 300 & 50   & 50        &  Useable with caution\\
  100         & 10 $\times$ 300 & 50   & Very poor &  Unsuitable for scientific use\\
  $> 100$     &                 &      &           &  Not Retrieved\\
  \bottomrule
\end{tabular}
\end{minipage}
