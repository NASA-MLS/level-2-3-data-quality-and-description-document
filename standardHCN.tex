...
MLS observes \ch{HCN} emission lines within the 190-GHz radiometer. As noted in Section~\ref{sec:R2DutyCycling}, the 190\nobreakdash-GHz radiometer has been operated only intermittently since May 2024, typically for less than one week per month. When the \mbox{190-GHz} radiometer is turned off the \ch{HCN} status flags are set to indicate that the data should not be used. The spectral signature of \ch{HCN} in the MLS radiances is weak, a relatively strong smoothing constraint is required to obtain meaningful retrievals, and averaging (e.g., weekly zonal means) is recommended.
...
% The precision and scatter are essentially identical in all of versions 4, 5 and 6, so this table is not changed from earlier editions of this document
  ...
  \tableheaderbox{Resolution%
    \footnote{Vertical and along-track horizontal resolutions; cross-track horizontal resolution is \mytexttilde9\,km.}\\V $\times$ H\\km} &
  \tableheaderbox{Single-Profile\\Precision%
    \footnote{The precision (1$\sigma$) shown is the estimated precision (\texttt{L2gpPrecision}); the observed scatter is about 80\% of this value.}\\pptv} &
  \tableheaderbox{Accuracy\\ \\pptv} &
  \textbf{Comments}\\
  \midrule
