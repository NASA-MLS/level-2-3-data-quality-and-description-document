...
The v4 SO\cs2 retrieval has two significant changes from v3. The first is
the use of a frequency squared extinction treated as a MIF-like quantity.
Each radiometric scan retrieves its own extinction
quantity that is not coupled vertically or horizontally to the radiances
measured at other scan elevations or along track scan tangent positions.
The new approach was motivated by having
...
rejection algorithm. These changes have significantly
improved the convergence and quality performance of the retrieval system and
...
\item[Estimated Precision:] \screeningrule{Values with negative
  ...
  values which are sometimes enough to trigger the ``too much \emph{a
  ...
  observed, reflecting the detection of a plume.
\evenstatusrule
...
\end{center}
\caption{Time series overlay where MLS measures more than 500~ppbv at 100~hPa. Blue is version 3, and red is version 4.}
