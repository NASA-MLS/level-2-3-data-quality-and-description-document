...
background such as that from volcanic injections. Validation of SO\cs2
is published in \citet{PumphreyEtal14}.

\subsection*{Changes from v3}

\mycomment{This is more detail than given for other 240-GHz species, we
  should try to be consistent} The v4 SO\cs2 retrieval has two
significant changes from v3. The first is the use of a frequency squared
extinction treated as a MIF-like quantity.  Each radiometric scan
retrieves its own extinction quantity that is not coupled vertically or
horizontally to the radiances measured at other scan elevations or along
track scan tangent positions.  The new approach was motivated by having
better performance in clouds where forcing vertical and horizontal
coupling can cause errors due to inhomogeniety in the cloud field and
errors in the forward model which currently doesn't include scattering
effects. The other change is an improved cloud detection and radiance
...
\begin{figure}
\begin{center}
\includegraphics[bb = 0 0 1840 510, clip, width=0.8\linewidth]{figures/mls_so2_v3-v4_prfl_comp_60n-90n}
\includegraphics[bb = 0 0 1840 470, clip, width=0.8\linewidth]{figures/mls_so2_v3-v4_prfl_comp_30n-60n}
\includegraphics[bb = 0 0 1840 470, clip, width=0.8\linewidth]{figures/mls_so2_v3-v4_prfl_comp_30s-30n}
\includegraphics[bb = 0 0 1840 470, clip, width=0.8\linewidth]{figures/mls_so2_v3-v4_prfl_comp_60s-30s}
\includegraphics[bb = 0 -50 1840 470, clip, width=0.8\linewidth]{figures/mls_so2_v3-v4_prfl_comp_90s-60s}
\end{center}
...
Figure~\ref{fig:avgmlsso2prflcomp} compares MLS v4.2 and v3.3. The atmosphere
typically has $\sim$ 0.1~ppbv SO\cs2, which is well below the MLS detection
limit. Both version show near zero SO\cs2. The small departures on the order
of a few ppbv from zero are caused by systematic errors in the MLS measurement
system. The larger -20~ppbv measured at 215~hPa is also a systematic artifact.

\subsection*{Resolution}
\onestandardavk{SO2}{SO\cs2}%

Based on Figure~\ref{fig:AvkSO2}, the vertical resolution for SO\cs2 is
$\sim$3\,km and the horizontal resolution is 170\,km. The horizontal
resolution perpendicular to the orbit track is 6\,km, the full width at half
maximum of the MLS antenna, for all pressures.

\subsection*{Precision}

The estimated precision for SO\cs2 is $\sim$4\,ppbv for all heights
...
performed on the MLS measurement system \citep{ReadEtal07}. The
accuracy is estimated to be $\sim$5\,ppbv for pressures less than 147\,hPa
increasing to $\sim$20\,ppbv at 215\,hPa. These may change for the v4 SO\cs2
product. The MLS team plans to repeat the v3 exercise with the v4 software
and release the results in a subsequent version of this document.
...
\item[Estimated Precision:] \screeningrule{Values with negative
  ...
  higher pressures (e.g., 215 and 147~hPa) will also have larger
  precision values which are sometimes large enough to trigger the ``too
  much \emph{a priori} influence'' flag. There will be an \emph{a
    priori} influence is present and the retrieved value is probably
  smaller than reality because the retrieval is being pulled towards the
  a priori value of zero. Nonetheless, this does not detract from the
  fact that greatly enhanced SO\cs2 is being observed, reflecting the
  detection of a plume.

\evenstatusrule
\item[Quality field:] \screeningrule{Only profiles having \texttt{Quality}\,
    \emph{greater} than 0.95 should be used.} As with water vapor, this
value represents where the quality versus yield sharply drops indicating
the transition from well fit profiles toward more poorly fit ones as shown
in figure~\ref{fig:so2datayields}
\item[Convergence field:] \screeningrule{Only profiles having
    \texttt{Convergence}\, \emph{less} than 1.1 should be used.}
\end{description}
...
zero. The atmospheric background SO\cs2 ($\sim$0.1~ppbv) cannot be
measured by MLS even with extensive data averaging because there are
...
These include Manam (Papua, New Guinea -- 3 events), Anatahan (Mariana Islands),
Sierra Negra (Galapogos Island), Soufriere Hills (Montserrat, 
West Indies -- 2 events),
Tunguraua (Ecuador), Rabaul (Papua New Guinea), Piton de la Fournaise
(Reunion Island), Jebel al-Tair (Yemen), Okmok (Alaska), Kasatochi
(Alaska), Dalaffilla (Ethiopia), Redoubt (Alaska), Sarychev (Kuril Islands,
Russia), Pacaya (Guatamala), and Merapi (Indonesia), Grimsvotn (iceland),
Puyehue-Cordon Caulle, (Chile), Nabro (Eritrea), Paluweh (Indonesia),
