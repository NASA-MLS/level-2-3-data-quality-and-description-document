...
Changes from \prevv to \thisv are minor and mostly affect O\cs3 and CO. A new
O\cs3 \emph{a priori} with new smoothing changes are incorporated and a
non-linear radiance model is used to calculate the signals in the high
spectral resolution digital autocorrellator spectrometer (DACS) channels for
the O\cs3 and CO lines. In addition to these changes, the accuracy threshold
for the non-linear radiance model calculations was reduced from 0.02K to 
best possible (i.e., 0K). These changes will have secondary impacts on SO\cs2.
...
\caption{A comparison of
  v2.2 (dark blue), v3.3 (light blue), v4.2 (orange) and v5.0 (red) SO\cs2 for
  Jan-Feb-Mar 2009
  ...
  bullets), measured variability (dotted) which includes atmospheric
  variability about the mean profile, and \emph{a priori} precision
  (dashed and diamonds).}
...
reported at 215\,hPa is also a systematic artifact. Changes from v2 through
to v4 involved handling of the background radiance signals which evolved
from an absorption continuum (v2) to an RHi continnuum (v3) to a MLS
minor frame absorption continuum with a frequency squared spectral dependence
(v4 and v5). Version 4 and 5 also uses improved cloud detection and radiance
screening for them.
...
that the data is biased toward the a priori value. Even though the changes
between v4 and v5 are minor, there is a change in the estimated precision.
An example of this is shown in Figure~\ref{fig:so2prec}. SO\cs2 precision
from v5 shows much less variability and less latitude dependence than
v4. Although only 100~hPa is shown here, it is similar for other stratospheric
levels and it also affects other species retrieved from the 240~GHz like
HNO\cs3. The change in behavior occurred as a result of reducing the forward
model accuracy threshold from 0.02K to ``best possible.''

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{figures/so2_2009d045_100hPa_precision}
\end{center}
\caption{A comparison of estimated precision for SO\cs2 from v4 (blue) and
v5(red) and 100~hPa.}
\label{fig:so2prec}
\end{figure}
...
underestimated due to \emph{a priori} influence which biases the value toward
...
\caption{An overlay of MLS measurement tracks on an OMI SO\cs2
  ...
  triangle). The OMI instrument has some non operational pixels also known
  as the row anomaly where measurements are not possible. It turns out that
  one of these row anomalies is perfectly aligned with the MLS track and
  therefore there are no coaligned OMI-MLS measurements. 
  The color scale indicates the SO\cs2 column measured by
...
Figure~\ref{fig:so2timeseries} shows a comparison between v5 and v4 for
a track showing the maximum SO\cs2 measured on 16~June 2009. As expected v5
and v4 show similar amounts of SO\cs2 at levels 215--68~hPa. Nominally the
SO\cs2 is concentrated between 147 and 100~hPa.
