...
\item Information on the accuracy of each product is given in
  Chapter~\ref{chap:Results}.  In the previous versions of this
  document, the numbers given were for the previous v3.3 dataset.  This
  document has now been updated with the specific \thisv accuracy
  estimates.
...
\thisv data, establishing them as ``Stage~4'' validated, defined as:
\begin{quote}\itshape
...
\texttt{L2gpPrecision}, which describe the value and precision (reported
at 1$\sigma$) of the data, respectively.  Data points for which
...
quantification of these uncertainties has been performed for the \thisv\
MLS products, updated from that described in the MLS validation papers
(which were based on v2.2).  The individual sections of
Chapter~\ref{chap:Results} detail the results of this quantification
product-by-product.
...
knowledge and/or design requirements.  Accordingly, the numbers reported
...
artifacts \citep{MillanEtal12}, and those products (based on
...
are expected to be applicable also to \thisv. The potential systematic
...
\parbox[m]{4.5em}{\bfseries\centering Day/night difference accuracy\footnote{Because of large biases in
...
retrievals of synthetic radiances; see \citet{SanteeEtal13} for details
...
relative to airborne \citep{SinghEtal03}, balloon-borne
\citep{KleinbohlEtal05}, and \mbox{ACE-FTS} satellite
\citep{HarrisonAndBernath13} CH\cs3CN measurements, as do results from
...
Atmosphere Research Satellite (UARS) \citep{LiveseyEtal01} (not
...
measurements have been quantified through a comprehensive set of
retrievals of synthetic radiances; see \citet{SanteeEtal08clo} for
...
MLS \thisv ClO data (see Table~\ref{tab:ClOSummary}).
...
\item Significant biases are present in both daytime and nighttime
  ...
  file available from the MLS web site.
\end{itemize}
...
channels considered in the retrieval. Tropospheric (215\,--\,100\,hPa) 
...
that is described above is generally effective in removing these artifacts.
\end{itemize}
...
for scientific use \\
0.0046 &   6.2   $\times$ 200 &  6000  &   $+$20\% to $+$50\%     &  \\
...
100    &   4.9   $\times$ 450 &  14     & $\pm$20\,ppbv and $\pm$24\% & \\
147    &   5.1   $\times$ 570 &  16     & $\pm$30\,ppbv and $\pm$30\% & \\
215    &   5.4   $\times$ 690 &  19     & $\pm$40\,ppbv and $\pm$30\% \\
...
data assimilation system (GEOS-5) \citep{RieneckerEtal07}, used in the
...
\citep{ReadEtal07}.  For pressures $\leq83$\,hPa the precisions are the
...
same approach has been used for \thisv) in \citet{ReadEtal07} and
\citet{LambertEtal07}. For pressures between 316\,--\,178\,hPa,
...
Please note the discussion of a potential drift in the MLS lower
...
information. MLS \thisv is a part of a satellite H\cs2O assessment
report to be published later this year.

\subsection*{Potential drift in lower stratospheric water vapor}
\label{sec:H2ODrift}

\citet{HurstEtal16} discuss how comparisons between MLS lower
...
and FPH\footnote{Cryogenic Frostpoint Hygrometer (CFH) and Frost Point
  Hygrometer (FPH)} instruments from 2004 to 2015 reveal signs of a
...
Comparisons with recently-released ACE-FTS \citep{BernathEtal04} data
...
measurements [\emph{Nedoluha}, private communication].
...
Note the discussion of the potential drift in MLS lower stratospheric
water vapor above.
...
0.004 & 10.8 $\times$ 585 & 81 & 23 & 0.1 & \\
0.010 &  8.8 $\times$ 725 & 55 & 17 & 0.1 & \\
0.021 &  8.0 $\times$ 745 & 41 & 19 & 0.1 & \\
0.046 &  7.4 $\times$ 540 & 35 & 17 & 0.1 & \\
0.10 &  5.8 $\times$ 745 & 24  & 14 & 0.1 & \\
0.21 &  3.6 $\times$ 670 & 19  & 11 & 0.1 & \\
...
The standard HNO\cs3 product is derived from the 240-GHz retrievals at
...
147\,hPa and slightly lower at 100\,hPa.  A summary of the estimated
precision, resolution (vertical and horizontal), and systematic
uncertainty of the \thisv HNO\cs3 measurements as a function of altitude
...
however, this procedure produces an upper limit on the precision-related
...
The effects of various sources of systematic uncertainty (e.g.,
instrumental issues, spectroscopic uncertainty, and approximations in
the retrieval formulation and implementation) on the MLS \thisv HNO\cs3
measurements have been quantified through a comprehensive set of
retrievals of synthetic radiances; see \citet{SanteeEtal07hno3} for
details of a similar analysis conducted on MLS v2.2 HNO\cs3 data.  The
overall systematic uncertainty, or accuracy, is calculated by combining
(RSS) the contributions from both the expected biases and the additional
scatter each source of uncertainty may introduce into the data.  In
aggregate, the factors considered in these simulations are estimated to
give rise to a total systematic uncertainty ranging from approximately
0.1 to 2.4\,ppbv, depending on the level, in the MLS \thisv HNO\cs3 data
...
\item[Quality:] \qualityrule{0.8} This threshold for \texttt{Quality}
  ...
  profiles in \thisv show a ``notch'', with unexpectedly low (often
  negative in the tropics) values at 100\,hPa and high values at
...
\item[Clouds:] \screeningrule{Clouds impact HNO\cs3 data in the UTLS,
    ...
    follows.}  Nonzero but even values of \texttt{Status} indicate that
...
\item[Outliers:] \screeningrule{Alternative screening approaches in the
    UTLS remove outliers while reducing ``false positives''} While the
  ...
  as well as many (but not all) with the ``notch'' structure between 147
...
\begin{itemize}
\item There is a persistent ``notch'' at 100\,hPa, noted in
  Figures~\ref{fig:zmv3v4diffs} and \ref{fig:outlierprofs}, that is most
  pronounced in the tropics but also apparent in midlatitudes.  \thisv
  HNO\cs3 at 147\,hPa is consistently slightly higher than that at
  ...
  feature is more pronounced than it was in \prevv, and it is reduced,
  but by no means eliminated, by following the recommended screening
  procedures.  This feature was persistent throughout \thisv testing,
...
\begin{itemize}
\item Diagnose and eliminate the unphysical notch (minimum) at
  100\,hPa.
\end{itemize}
...
described in \citet{PickettEtal06}. The estimated uncertainties, precisions,
...
\caption{Monthly zonal mean of retrieved HO\cs2 and its estimated
  ...
  profiles, while the average in panels (e)\,--\,(f) using \prevv data
...
\citep{ReadEtal07}. These quantified effects correspond to either
...
with more realistic HO\cs2 atmospheric profiles than for \prevv
estimates.  The HO\cs2 bias can be eliminated by taking day-night
...
\midrule
 ...
 0.046\,hPa     & 10    & 6                 & 0.01 &  Use day--night difference \\
 0.10\,hPa      & 7     & 10                & 0.2  &  Use day--night difference \\
 1.0 \,hPa      & 5     & 11                & 0.2  &  Use day--night difference \\
 10  \,hPa      & 4     & 50                & 14   &  Use day--night difference \\
...
\citep{ReadEtal07}.  These (updated) values are intended to represent
...
the range of pressures).  The average changes for upper stratospheric
HOCl between v3.3 and v4.2 are well within the quoted accuracy estimates
(which may be somewhat conservative).  The HOCl signal becomes too small
...
\item[Pressure range:] \screeningrule{10\,--\,2.2\,hPa}
  ...
  larger than about 10\,hPa make this product unsuitable for use in the
...
\item There are significant artifacts in the mean values (large negative
...
2.2 to 10   &  10 & 10    &   6  & 40\,--\,80 & \texttildelow150 & Some averaging required \\ 
...
\paragraph*{Column values} The estimated precisions for the \thisv MLS 
...
calibration, spectroscopy, etc., through the (\thisv) measurement system.  The 
values shown here are intended to represent 2$\sigma$ estimates of accuracy.  Comparison of MLS O\cs3 with
well-established data sets shows no evidence of significant biases overall, although 
the oscillations in the UTLS at low latitudes lead to some systematic effects which can reach 20-40\%, depending on the pressure level. 
...
on the MLS website under ``Publications''.  In particular, see the comprehensive work by \citet{HubertEtal16} (more related information further below). In recent years, validation studies have focused more on longer-term changes or drifts (with good stability being observed versus other reliable measurements) and on the UTLS region.
...
the multi-instrument mean values as well as to SAGE II ozone.  The temporal stability of the Aura MLS ozone dataset has been shown to be very good, in comparison to lidar datasets (\citet{NairEtal12}). The more recent
work by \citet{HubertEtal16} includes both the lidar network and the ozonesonde network as references for a comprehensive satellite data intercomparison study. The latter authors show that average biases between MLS and latitudinally-binned data from lidar and ozonesonde sites across the globe is typically within 5\% or better, with poorer behavior (known vertical oscillations) at low latitudes in the UTLS.  In terms of drifts, the Aura MLS ozone dataset is shown (in the above work) to be very stable, a term also applied only to the SAGE II dataset; MLS exhibits drifts with respect to the ground-based networks that fall within 1.5 to 2\% per decade, with zero drift encompassed by the error bars (i.e. non-significant drifts), at least in the middle stratosphere. The Aura MLS data, in combination with older datasets, provides a critical tool for the study of global O\cs3 in a changing climate and into the expected O\cs3 recovery period, as anthropogenic ozone depleting substances decline in concentration. 
...
\item[Oscillations in tropical UTLS ozone:] There has been some improvement 
  ...
  (but not eliminating) systematic vertical oscillations. Further characterization 
  of MLS tropical UTLS data, in particular, is warranted
...
0.02 & 5.5 $\times$ 200 & 1.2  & 300    & 0.2  &  50   & \\
0.05 & 5.5 $\times$ 200 & 0.8  & 150    & 0.2  &  30   & \\
0.1  & 4 $\times$ 400   & 0.5  &  60    & 0.2  &  20   & \\
0.2  & 3 $\times$ 450   & 0.4  &  30    & 0.15 &  10   & \\
0.5  & 3.5 $\times$ 550 & 0.3  &  20    & 0.2  &  10   & \\
1    & 3 $\times$ 500   & 0.2  &  7     & 0.3  &  10   & \\
2    & 3.5 $\times$ 450 & 0.15 &   3    & 0.3  &   7   & \\
5    & 3 $\times$ 450   & 0.15 &   2    & 0.4  &   7   & \\
10   & 3 $\times$ 500   & 0.1  &   2    & 0.4  &   6   & \\
22   & 2.5 $\times$ 400 & 0.1  &   2    & 0.25 &   5   & \\
46   & 2.5 $\times$ 350 & 0.06 &   3    & 0.2  &   8   & \\
68   & 2.5 $\times$ 350 & 0.04 &   4    & 0.1  &   7   & \\
100  & 3 $\times$ 300   & 0.03 & 15--25 & \multicolumn{2}{c}{$\left[+0.005 +7\%\right]$}  \\
150  & 3 $\times$ 400   & 0.02 & 5--70  &  \multicolumn{2}{c}{$\left[+0.005 +7\%\right]$} \\
215  & 3.5 $\times$ 350 & 0.02 & 5--100 & \multicolumn{2}{c}{$\left[+0.01 +10\%\right]$} \\
261 & 3.5 $\times$ 400 & 0.03 & 5--100 &
\multicolumn{2}{c}{$\left[+0.02 +10\%\right]$} & See note
\footnote{Positive bias in the UT,
  but the mean annual variation is nevertheless well behaved
  versus tropical sonde data.}\\
316  & 2.5 $\times$ 550 & 0.03 & ---    & ---  & ---   & Not recommended \\
1000\,--\,464 & ---     & ---  & ---    & ---  & ---   & Not retrieved \\
\bottomrule
\end{tabular} 
\end{minipage} 
...
Also, users should note the possibility of a potential drift in the MLS
...
injections. Validation of SO\cs2 is published in \citet{PumphreyEtal15}.
...
Accuracy is estimated to be 10\,--\,15\,ppbv for pressures less than
...
A substantial study has been made of sources of systematic error in MLS
\thisv measurements, similar to that which was done as a part of the
validation of v2.2. The accuracy of the \thisv temperature measurements
is estimated both by modeling the impact of uncertainties in measurement
...
found that it did not close forward model and measured radiances as
...
predicted vertical structure is similar to observed biases relative
...
The last four profiles of each day show increased rates of outliers
relative to GEOS-5 temperature, compared to other profile positions in
the day in version~4.20 (the problem is fixed in version~4.22).  These outliers can be
as large as 50\,K at at some levels.  A recommendation to discard the
last 4 profiles of each day has been added to recommended screening for
v4.20 (not needed for v4.22 and later versions).
...
version~5~\citep{RieneckerEtal07} (GEOS-5), the European Center for
...
\midrule
$<$0.001\,hPa      & ---       & ---       &  ---             & --- & --- & Unsuitable for scientific use \\
0.001\,hPa         & $\pm$3.6  & $\pm$3.5   & 13--14 $\times$ 290 & $+$2\,$\pm$\,1      & $-$9     & \\
0.01\,hPa          & $\pm$3.3  & $\pm$3     & 10--11 $\times$ 260 & $+$2\,$\pm$\,1      & $-$2\,to\,0 & \\
0.1\,hPa           & $\pm$2.4  & $\pm$2.7   & 6      $\times$ 170 & $+$3\,$\pm$\,1.2    & $-$8\,to\,0 & \\
0.316\,hPa         & $\pm$1.3  & $\pm$1.5   & 8      $\times$ 170 & $+$2\,$\pm$\,0.8    & $-$7\,to\,$-$4& \\
1\,hPa             & $\pm$1.2  & $\pm$1.3   & 6.8    $\times$ 165 & $+$1\,$\pm$\,0.5    &  0\,to\,$+$5 & \\
3.16\,hPa          & $\pm$0.8  & $\pm$0.8   & 5.5    $\times$ 165 & $+$2\,$\pm$\,1.2    & $+$1 & \\
10\,hPa            & $\pm$0.6  & $\pm$0.7   & 4.1    $\times$ 165 & $-$1\,$\pm$\,0.5      & $-$1\,to\,0& \\
14.7\,hPa          & $\pm$0.6  & $\pm$0.7   & 3.9    $\times$ 165 & $+$2.7\,$\pm$\,0.9    &  0\,to\,$+$1 & \\
31.6\,hPa          & $\pm$0.6  & $\pm$0.6   & 3.6    $\times$ 165 & $+$0.7\,$\pm$\,0.7    & $-$2\,to\,0 & \\
56.2\,hPa          & $\pm$0.6  & $\pm$0.7   & 3.7    $\times$ 165 & $-$1.5\,$\pm$\,0.8    & $-$2\,to\,0 & \\
100\,hPa           & $\pm$0.7  & $\pm$1.0   & 4.7    $\times$ 165 & $+$1.8\,$\pm$\,1.2    &  0\,to\,$+$1  & \\
215\,hPa           & $\pm$0.8  & $\pm$1.2   & 4.2    $\times$ 165 & $-$0.8\,$\pm$\,1.4 & $-$2.5\,to\,$-$1.5   & \\
261\,hPa           & $\pm$0.7  & $\pm$1.5   & 4.2    $\times$ 167 & 0.4\,$\pm$\,2.4      & $-$2\,to\,0   & \\
1000\,--\,316\,hPa & ---       & ---      & ---              & ---           & --- & Unsuitable for scientific use\\
\bottomrule
\end{tabular}
\end{minipage}
...
% \midrule
% $<$0.001\,hPa      & ---       & ---       &  ---             & --- & --- & Unsuitable for scientific use \\
% 0.001\,hPa         & $\pm$3.6  & $\pm$3.5   & 13--14 $\times$ 290 & $+$2\,$\pm$\,3    & $-$9     & \\
...
% 31.6\,hPa          & $\pm$0.6  & $\pm$0.6   & 3.6    $\times$ 165 & $+$1\,$\pm$\,2    & $-$2\,to\,0 & \\
% 56.2\,hPa          & $\pm$0.6  & $\pm$0.7   & 3.7    $\times$ 165 & $-$1\,$\pm$\,2    & $-$2\,to\,0 & \\
% 100\,hPa           & $\pm$0.7  & $\pm$1.0   & 4.7    $\times$ 165 & $+$2\,$\pm$\,2    &  0\,to\,$+$1  & \\
% 215\,hPa           & $\pm$0.8  & $\pm$1.2   & 4.2    $\times$ 165 &  $-$1.5\,$\pm$\,4 & $-$2.5\,to\,$-$1.5   & \\
% 261\,hPa           & $\pm$0.7  & $\pm$1.5   & 4.2    $\times$ 167 &  0\,$\pm$\,5      & $-$2\,to\,0   & \\
% 1000\,--\,316\,hPa & ---       & ---      & ---              & ---           & --- & Unsuitable for scientific use\\
% \bottomrule
% \end{tabular}
% \end{minipage}
