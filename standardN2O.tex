...
\texttt{N2O-64O} data such as the resolution, precision, quality and
...
\mycomment{But what about \texttt{N2O-190}? Can we say anything?}

\subsection*{Data screening}
\label{sec:N2OScreening}
\begin{description}
\item[Pressure range:] \screeningrule{68\,--\,0.46\,hPa}
  ...
  signals, but see the note under ``Artifacts'' for issues above
  0.1\,hPa. \mycomment{Check with Alyn}
...
\item[Clouds:] \screeningrule{Clouds can impact the N\cs2O product at
  ...
  low) values at 100\,hPa \mycomment{But we're not recommending that}
  (and 147\,hPa, not recommended for scientific use).  Such cases are
  not always detected by the \texttt{Quality} and \texttt{Convergence}
  flags, and the cloud bits of the \texttt{Status} field are too blunt a
  tool to identify these cases, needlessly discarding reasonable data.
  We recommend checking for the rare occurrence of N\cs2O values greater
  than 350\,ppbv on the 68\,hPa MLS retrieval level in order to remove
  significant outliers from the 100\,--\,46\,hPa data.
...
removed by screening), there are occasional nonphysical values of N\cs2O
at other levels in the \thisv data, and screening using the convergence
and quality fields (see above) is recommended to remove the majority of
these data points. Improvements in the handling of radiances affected by
clouds in \thisv have reduced the frequency of outliers compared to
previous versions.
