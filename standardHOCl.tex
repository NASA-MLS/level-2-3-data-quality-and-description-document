...
As noted in Section~\ref{sec:R2DutyCycling}, since May 2024 the \mbox{190-GHz} radiometer is operated only intermittently. When that radiometer is turned off and H\cs2O measurements are not available, \emph{a priori} information for H\cs2O (based on climatological values) is used in the HOCl retrievals, adversely affecting their quality; further details are given in Section~\ref{sec:HOClScreening}. 
...
An algorithm to retrieve daily zonal means of HOCl by first averaging the radiances has been developed by the MLS team, building on the work described in \citet{MillanEtal:2012} and \citet{MillanEtal:2015} for BrO and HO\cs2, respectively. The resulting alternative dataset reduces the variability slightly and will be available, along with associated documentation, for \thisv from the GSFC DISC soon; it is expected to be very similar to the (\prevv) HOCl dataset used by \citep{FroidevauxEtal:2022:HOCl}. 

\subsection{Changes from \prevv}
There were no significant algorithmic changes relating directly to HOCl for the \thisv retrievals. However, given the small level of HOCl spectral radiance signal, the retrieved HOCl abundances can be sensitive, indirectly, to even minor changes in the retrieval system, such as changes in temperature or tangent pressure. However, please see the comments in the Introduction above as well as further below regarding some retrieval issues when the \mbox{190-GHz} radiometer is turned off intermittently (since May 2024).  \mycomment{If the R2 cycling impacts HOCl, this would be the place to discuss it}
...
\begin{itemize}
  ...
  \item As discussed in Section~\ref{sec:HOClScreening}, biases in HOCl values are present at some levels when coincident measurements from the \mbox{190-GHz} radiometer are not available and H\cs2O \emph{a priori} information is used in the HOCl retrieval. Depending on the specific goals of any study based on these measurements, additional HOCl data quality filtering may be necessary for days when the \mbox{190-GHz} radiometer is off.
\end{itemize}
\begin{table}
