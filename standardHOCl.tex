...
As noted in Section~\ref{sec:R2DutyCycling}, since May 2024 the \mbox{190-GHz} radiometer is operated only intermittently. When that radiometer is turned off and \ce{H2O} measurements are not available, \emph{a priori} information for \ce{H2O} (based on climatological values) is used in the HOCl retrievals, adversely affecting their quality; further details are given in Section~\ref{sec:HOClScreening}. 
...
An algorithm to retrieve daily zonal means of HOCl by first averaging the radiances has been developed by the MLS team, building on the work described in \citet{MillanEtal:2012} and \citet{MillanEtal:2015} for BrO and \ce{HO2}, respectively. The resulting alternative dataset reduces the variability slightly and will be available, along with associated documentation, for \thisv from the GSFC DISC soon; it is expected to be very similar to the (\prevv) HOCl dataset used by \citep{FroidevauxEtal:2022:HOCl}. 
...
\begin{itemize}
  ...
  \item As discussed in Section~\ref{sec:HOClScreening}, biases in HOCl values are present at some levels when coincident measurements from the \mbox{190-GHz} radiometer are not available and \ce{H2O} \emph{a priori} information is used in the HOCl retrieval. Depending on the specific goals of any study based on these measurements, additional HOCl data quality filtering may be necessary for days when the \mbox{190-GHz} radiometer is off.
\end{itemize}
\begin{table}
