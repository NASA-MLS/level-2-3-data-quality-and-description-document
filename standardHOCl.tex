...
An algorithm to retrieve daily zonal means of HOCl by first averaging the radiances has been developed by the MLS team, building on the work described in \citet{MillanEtal:2012} and \citet{MillanEtal:2015} for BrO and \ch{HO2}, respectively. The resulting alternative dataset reduces the variability slightly and will be available, along with associated documentation, for \thisv from the GSFC DISC soon; it is expected to be very similar to the (\prevv) HOCl dataset used by \citep{FroidevauxEtal:2022:HOCl}. 

\subsection{Changes from \prevv}
There were no significant algorithmic changes relating directly to HOCl for the \thisv retrievals. However, given the small level of HOCl spectral radiance signal, the retrieved HOCl abundances can be sensitive, indirectly, to even minor changes in the retrieval system, such as changes in temperature or tangent pressure. However, please see the comments in the Introduction above as well as further below regarding some retrieval issues when the \mbox{190\nobreakdash-GHz} radiometer is turned off intermittently (since May 2024).  \mycomment{If the R2 cycling impacts HOCl, this would be the place to discuss it}

A comparison plot showing zonal average upper stratospheric HOCl contours (from 10 to 2\,hPa) and differences between the two data versions for a typical month (April 2009) is provided in Figure~\ref{fig:MLSv5v6HOClContour}. The average \thisv HOCl abundances are slightly larger than the \prevv retrievals, typically by about 10 to 15\,pptv (or 10 to 15\%); this has removed some of the small negative values that often occurred in \prevv HOCl averages at the highest southern latitudes, between 5 and 10\,hPa.  The estimated precision values are essentially unchanged from \prevv.
...
The estimated single-profile precision reported by the Level~2 software is about 300 to 400\,pptv in the upper stratosphere. A more useful number of 5 to 10\,pptv is quoted in Table~\ref{tab:HOClSummary} for the typical precision of a 10\degsym{}-latitude-resolution weekly zonal means for this product.
