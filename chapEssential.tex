...
make use of Aura MLS data for scientific study.  The bulk of the
geophysical products described in this document (this chapter through
Chapter~\ref{chap:Results}) are produced by the ``Level~2''
algorithms, and briefly summarized in Table~\ref{tab:ProductSummary}.
In addition, Chapter~\ref{chap:Level3} (added in version 4.x-4.0 of
this document) describes ``Level~3'' data products derived from the
Level~2 data that provide easier access to collections of MLS
observations (e.g., daily zonal means, monthly polar vortex averages)
to facilitate simpler studies using MLS observations.
...
updates findings from these papers for \thisv, and gives more general
information on the use of MLS data.  As always, those wishing to use
MLS data are advised to consult the MLS science team concerning their
intended use.

\input{producttable}

In addition to describing the Level~2 data and their quality (this
chapter and Chapter~\ref{chap:Results}), Chapter~\ref{chap:Background} of
this document also gives a brief outline of the algorithms used to
generate the ``Level~2'' data (geophysical products reported along the
instrument track) from the input ``Level~1'' data (calibrated
microwave radiance observations).  Finally, Chapter~\ref{chap:Level3}
provides information on MLS Level~3 products.
...
earlier MLS instrument is given in \citet{WatersEtal:1999}.  The
...
are available from the MLS web site (\url{https://mls.jpl.nasa.gov/}).
...
Chapter~\ref{chap:Results} on a product-by-product basis.
...
software is version~5.00, producing files labeled \texttt{v05-00}.
...
temperature will be referred to as v5.01, v5.02, etc.  This version of
...
As discussed above, a complete set of MLS validation papers describe
...
\item[Partial amelioration of drifts in 190-GHz products (H\cs2O,
  ...
  H\cs2O product has been slowly drifting since \texttildelow2010, at a rate
  of between 0.2--1\%/year, depending on what record is compared to
  (mainly frostpoint sonde vs.\ ACE-FTS) and the altitude range of
  interest.  The MLS team has identified a slow change in instrument
  calibration that accounts for some of this drift, and is corrected
  for in \thisv.  The extent to which this correction fully accounts
  for the observed drift remains to be studied as the \thisv
...
\item[Correction of dry bias in upper tropospheric H\cs2O:] As part of
  ...
  MLS team has also identified and corrected the source of a
  \texttildelow20\% dry bias below the tropopause.
\item[Extension of N\cs2O product to 100\,hPa:] The \thisv\ algorithms
  include a new retrieval for N\cs2O that reduces issue seen at
  ...
  affects N\cs2O, and unphysical negative trends in this product in
...
\item[Increased vertical range for some species:] A new forward model
  ...
  in the upper mesosphere.  The MLS team is investigating whether
  these improvements are sufficient to increase the vertical range
...
response to the larger changes detailed above.  Note that the
threshold values of ``\texttt{Quality}'' and ``\texttt{Convergence}''
(see below) to be used in data screening have been updated for some
products.  All the standard product files (apart from IWC, see below)
also include the \emph{a priori} information used in the retrieval (as
an additional ``swath'', see below).
...
linear system \citep{LiveseyEtal:2006}.  Values around unity indicate good
...
For \thisv, the decision was taken to assign the band 3 (190\,GHz,
\texttt{N2O-190}) retrieval as the standard product from launch,
wherease the band 12 (640\,GHz, \texttt{N2O-640}) retrieval is now
only available in the \texttt{N2O-640} swath in the \texttt{L2GP-DGG}
file.
