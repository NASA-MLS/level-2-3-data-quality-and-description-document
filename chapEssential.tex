...
\rcsInfo $Id: chapEssential.tex,v 1.4 2014/11/19 20:56:02 livesey Exp $
...
temperature data), cloud ice water content and partial cloud ice water
path, all described as functions of pressure.  In \thisv these profiles
are mostly output on a grid that has a vertical spacing of six surfaces
per decade change in pressure (\texttildelow2.5\,km), thinning out to
three surfaces per decade above 0.1\,hPa.  Exceptions to this are water
vapor, temperature, ozone and relative humidity which are on a finer 12
per decade grid from 1000\,hPa to 1\,hPa.  Cloud ice water content is
also reported on this fine grid, though these profiles do not extend to
the stratosphere and mesosphere.  The OH product maintains a 6 per
decade grid spacing into the upper mesosphere.  Horizontally the
profiles are spaced by 1.5\degsym\ great-circle angle along the orbit,
which corresponds to about 160\,km.  The true vertical and along-track
...
\item[Improved composition profiles in cloudy regions:] For the earlier
  ...
  also an appreciable simplification of the steps users need to take to
  screen out the most cloud-contaminated measurements in their analyses,
  and the effectiveness that screening.
...
\item[Improvements to O\cs3 and HCN:] The MLS O\cs3 and HCN products
  ...
  ``phases'' dedicated to obtaining more accurate estimates for this
...
\item[Improvements to H\cs2O:] The MLS H\cs2O product is retrieved in
  ...
  calculations in this initial retrieval has led to improvements in the
  convergence etc. of the second phase, and thus to improvements in the
  H\cs2O product. \mycomment{Is this level of detail needed?  Are these
    issues covered to this level of detail in the H\cs2O section?  If
    not, perhaps we don't need so much detail here.}

\item[New methanol product:] This product, obtained from measurements in
  the 640- and 190-GHz spectral regions (also from a dedicated retrieval
...
\begin{table}
  \caption{Additional swaths in specific standard product files.\label{tab:SwathExtras}}\small
\begin{tabular}{>{\ttfamily}l>{\ttfamily}ll}
  ...
  240\,GHz radiometers\\
  IWC & IWP & partial Ice Water Path.  (This file has \emph{a priori} swath) \\
...
resolution in the upper stratosphere and mesosphere.  See the
...
\item The ground-based instrument used for MLS OH validation
  ...
  ground-based data.
\end{enumerate}
We have performed the \texttildelow30-day OH measurements each
August/September during 2011 through 2014 and plan to continue them in
the coming years.
