...
As discussed above, a complete set of MLS validation papers describes the validation state of the earlier v2.2\emph{x} data. The majority of the v2.2\emph{x} MLS data products have, accordingly, completed ``Stage~3 Validation'' defined\footnote{See \url{https://science.nasa.gov/earth-science/earth-science-data/data-maturity-levels/}} as:
%
\begin{quote}
...
\section{A note on MLS observations from May 2024 and later}
\label{sec:R2DutyCycling}

In contrast with the MLS receivers at 118, 240, and 640 GHz, which have exhibited no substantive changes in behavior in the two decades since launch, the MLS 190-GHz receiver, used to measure water vapor, nitrous oxide, nitric acid in the upper stratosphere, and hydrogen cyanide, has shown clear signs of aging starting around 2010.  One consequence of this aging has been a slow (\mytexttilde2\%/decade) drift in a key calibration parameter, the implications of which are discussed in \cite{LiveseyEtal:2021:Drifts} (and discussed within this document in Section~\ref{sec:V6N2Owork} and the product-specific sections of Chapter~\ref{chap:Results} for water vapor, nitrous oxide, and nitric acid \mycomment{others}).  This aging also has implications for the total operating life expected from this receiver.  In 2024, the MLS team determined that, to ensure as long a record as possible, the operation of the 190-GHz receiver should be duty cycled. Thus, since May 2024, it has been operated only about one week per calendar month to conserve the remaining operational lifetime.  Figure~\ref{fig:190GHzCalendar} details the schedule for MLS 190-GHZ activation.  
