\chapter{Essential reading for users of MLS version~3.3 and~3.4 data}
\rcsInfo $Id: chapEssential.tex,v 1.6 2013/05/14 20:29:30 livesey Exp $
...
produced by version~3.3 and~3.4 (jointly referred to as \thisv, hereafter) of the data
processing algorithms for the EOS Microwave Limb Sounder (MLS)
instrument on the Aura spacecraft.  The intended audience is those
wishing to make use of EOS MLS data for scientific study.  The
geophysical products described in this document are all produced by the
``Level~2'' algorithms, and briefly summarized in
Table~\ref{tab:ProductSummary}.

The \thisv MLS data are the third `public release' of MLS data, the first
being version~1.5 \citep{EOSMLSV15Quality}, and the second version~2.2.
The v2.2 data are described in a series of validation papers published
in a special issue of the \emph{Journal of Geophysical Research} in
2007/2008.  This document updates findings from these papers for
version~3.3, and gives more general information on the use of MLS data.
As always, those wishing to use MLS data are strongly advised to consult
the MLS science team concerning their intended use.
...
data described here are reviewed in
\citet{LiveseyEtal06,ReadEtal06,SchwartzEtal06,WuEtal06}\@.  Other
papers describe the calibration and performance of the various aspects
of the MLS instrument \citep{JarnotEtal06,Pickett06,CofieldAndStek06}
and the MLS ground data system \citep{CuddyEtal06}.  The detailed
validation of the MLS v2.2 dataset is described in a collection of
papers in the `Aura Validation' special issue of JGR-Atmospheres (papers
published in 2007 and 2008).  These are cited in the sections of
...
Chapter~\ref{chap:Results} describes the data quality to be expected for
``Standard'' products from the MLS instrument for \thisv.  These are
observations of vertical profiles of the abundance of BrO, CH\cs3Cl (a
new product on \thisv), CH\cs3CN, ClO, CO, H\cs2O, HCl, HCN, HNO\cs3,
HO\cs2, HOCl, N\cs2O, O\cs3, and OH and SO\cs2, along with temperature,
geopotential height, relative humidity (deduced from the H\cs2O and
temperature data), and cloud ice water content, all described as
functions of pressure.  In \thisv these profiles are mostly output on a
grid that has a vertical spacing of six surfaces per decade change in
pressure (\texttildelow2.5\,km), thinning out to three surfaces per decade
above 0.1\,hPa.  Exceptions to this are water vapor, temperature, ozone
and relative humidity which are on a finer 12 per decade grid from
1000\,hPa to 1\,hPa.  Cloud ice water content is also reported on this
fine grid, and profiles do not extend to the stratosphere and
mesosphere.  The OH product maintains a 6 per decade grid spacing into
the upper mesosphere.  Horizontally the profiles are spaced by
1.5\degsym\ great-circle angle along the orbit, which corresponds to
about 160\,km.  The true vertical and horizontal resolution of the
products is typically somewhat coarser than the reporting grid described
here.  For some of the products, the signal to noise ratio is too low to
yield scientifically useful data from a single MLS profile observation.
In these cases, some form of averaging (e.g.,\ weekly maps, monthly
zonal means etc.) will be required to obtain more useful results.
...
software is version~3.41, producing files labeled \texttt{v03-41}.  Any
minor `bug fix' updates will be referred to as v3.42, v3.43, etc.  This
document is intended to be applicable to any v3.3$x$ and v3.4$x$ MLS
data.
...
\section{Differences between MLS \thisv data and earlier v2.2 data}
\label{sec:V3vsV2Overview}

All of the MLS data products have been updated from the earlier v2.2
algorithms.  Significant updates are detailed below.
\begin{description}
\item[Changes to vertical grids:] Most products are reported on a
  vertical grid spaced at 6 surfaces per decade change in pressure
  (\texttildelow2.5\,km spacing), thinning out to 3 surfaces per decade at
  pressures less than 0.1\,hPa.  As with v2.2, selected products are
  reported on a higher resolution grid at lower altitudes, spaced at 12
  surfaces per decade (\texttildelow1.3\,km).

  In \thisv, these `high resolution' products transition back to the
  regular 6-surfaces-per-decade grid at 1.0\,hPa (v2.2 transitioned at
  22\,hPa).  As with v2.2, H\cs2O, Temperature, geopotential height, and
  relative humidity are reported on this grid.  In addition, O\cs3 is
  now reported on this `high resolution' grid in \thisv.

  \sloppypar\item[Amelioration of biases in upper tropospheric CO and
  HNO\cs3:] Significant biases in these products at 215\,hPa (and partly
  at 146\,hPa) have been ameliorated.  This has been accomplished by
  updates in spectroscopy, a change in the manner in which background
  radiance is modeled, and neglecting information from a small number of
  channels (in the 240-GHz spectral region) that were giving rise to
  retrieval problems.

\item[Extension of the vertical range for O\cs3:] In addition to being
  reported on a higher resolution vertical grid, the O\cs3 product now
  contains scientifically useful values at 261\,hPa, in contrast to v2.2
  where retrievals were useful only at pressures of 215\,hPa or less.

\item[Reduction of biases in ClO:] Biases in lower stratospheric ClO
  observations have been significantly reduced, but still need to be
  accounted for as described in the ClO section.

\item[New methyl chloride product:] CH\cs3Cl is now retrieved from
  spectral signatures in the 640-GHz region (partly responsible for the
  lower stratosphere ClO bias).

\item[Change in standard N\cs2O product (post June 26, 2013 only,
  version 3.42 onwards):] In response to the aging of the MLS 640\,GHz
  ``band~12'' signal chain, the Level~2 software was modified to use the
  190-GHz ``band~3'' signals as the primary source of the N\cs2O
  standard product for all observations after June 26, 2013.  We
  anticipate that later versions will use the band~3 signals
  exclusively.
\end{description}
...
response to the larger changes detailed above.

The improvements in the 240-GHz species (O\cs3, HNO\cs3, and CO in the
upper troposphere and lower stratosphere) partly derive from changes in
the modeling of background radiances (as described above).  An
unfortunate side effect of this change is that these measurements are
more sensitive to clouds than in v2.2.  Additional screening rules for
these products must be considered, as described in
chapter~\ref{chap:Results}.

Also note that the threshold values of `Quality' and `Convergence' to be
applied in data screening have been updated for all products.

\section{EOS MLS file formats, contents, and first order
  quality information}
...
(GSFC-DISC, see {\small\verb+http://disc.gsfc.nasa.gov/+}).  The
standard and diagnostic products are stored in the EOS MLS \emph{Level 2
  Geophysical Product} (L2GP) files.  These are standard HDF-EOS
(version~5) files containing swaths in the Aura-wide standard format.
For more information on this format see \citet{AuraFileConventions}.  A
sample reading function for the Interactive Data Language (IDL, version
6.1 or later required), known as \texttt{readl2gp.pro} may have been
supplied with the data and is available from the \emph{Open Channel
  Software Foundation} (\verb+http://www.openchannelsoftware.org/+).  A
reader for MATLAB (\texttt{readL2GP.m}) is also available at the same
...
\begin{quote}
\verb+MLS-Aura_L2GP-<product>_v03-41_<yyyy>d<ddd>.he5+
\end{quote}
...
January). The files contain an HDF-EOS swath given the same name as the
product.  In addition, the standard O\cs3 product files also contain
swaths describing column abundances, and the standard Temperature file
contains additional swaths describing tropopause pressure.  As some L2GP
files contain multiple swaths, it is important to ensure that the
correct swath in the \texttt{L2GP} files is requested from the file.  In
the case where the `default' swath is requested (i.e., no swath name is
...
for all products.  For example, ``\texttt{O3}'' comes before
%
``\verb*+O3 column-GEOS5+''.
%
Likewise, for temperature, the standard ``\texttt{Temperature}'' product
will be read in preference to the ``\texttt{WMOTPPressure-MLS}'' or
``\texttt{WMOTPPressure-GEOS5}'' swaths that give tropopause pressures
(note that, as with v2.2, these names are different from the equivalent
products in v1.5).
...
negative \emph{should not be used}, as this flags that the resulting
precision is worse than 50\% of the \emph{a priori} precision,
...
the number of seconds elapsed (including from five to seven subsequent
leap seconds to date) since midnight universal time on 1~January 1993.

\section[`Quality', `Convergence' and `Status']{Additional quality
  control information described in the `Quality', `Convergence' and
  `Status' fields}
...
Larger values of \texttt{Quality} generally indicate good radiance fits
and therefore trustworthy data. Values of \texttt{Quality} closer to
zero indicate poorer radiance fits and therefore less trustworthy data.
The value of \texttt{Quality} to be used as a ``threshold'' for
rejecting data in scientific studies varies from product to product, and
...
alone will never cause a profile to be marked as not to be used).  The
implications of this vary from product to product and, more importantly,
height to height.  For example, situations of ``low cloud'' typically
have very little impact on the quality of stratospheric data.  Further
details of the implications of these flags are given later in this
document on a product by product basis.
...
\end{equation}
The \emph{a priori} profile for each MLS observation is available from
the \texttt{L2GP-DGG} files.  These files (one file per day, as for the
product files) contain swaths named according to the product, with the
suffix `\texttt{-APriori}' (note the hyphen).  Examples are
`\texttt{Temperature-APriori}' and `\texttt{O3-APriori}'.
...
\begin{quote}
\verb+MLS-Aura_L2AK-<product>-<case>_v03-30_0000d000.txt+
\end{quote}
...
web site at
\begin{quote}
\verb+http://mls.jpl.nasa.gov/data/ak/+
\end{quote}
...
vertical profiles of mixing ratio (or temperature) as a function of
...
standard HCl product, even on days for which it was active (as with the
earlier v2.2 algorithms).  For days prior to the 16~February 2006
deactivation of band~13, and the few subsequent days when band~13 has
been (or will be) reactivated, the \thisv algorithms also produce a second
HCl product (in the \texttt{HCl-640-B13} swath in the \texttt{L2GP-DGG})
file which includes the band~13 radiances, giving a product with
improved precision and resolution in the upper stratosphere and
mesosphere.  See the section~\ref{sec:standardHCl} for more information,
including a list of the band~13 reactivation days to date.
...
radiometers is provided by a methanol laser (pumped by a CO\cs2 laser). In
December 2009, following more than five years of operation, this laser
began to show signs of aging and was temporarily deactivated (prior to
the 2004 Aura launch, the expected lifetime of this laser was only two
years).
...
We have performed the 30-day OH measurement during 2011 and 2012 and
plan to continue it in the coming years as the solar maximum approaches
(expected to be in 2013\,--\,2015). However, due to the aging of the
instrument and the rather long suspension period during 2009\,--\,2011,
``new'' OH data acquired since 2011 are significantly noisier than the
previous years and likely require additional calibration.  We have been
examining the data carefully.  The release of the ``new'' OH data for
scientific use will depend on the results of this continuing effort.
