...
\rcsInfo $Id: chapEssential.tex,v 1.1 2014/08/15 17:07:41 livesey Exp $
...
EOS MLS data for scientific study.  The geophysical products described
...
use MLS data are strongly advised to consult the MLS science team
concerning their intended use.
...
observations of vertical profiles of the abundance of BrO, CH\cs3Cl,
CH\cs3CN, CH\cs3OH (a new product on \thisv) ClO, CO, H\cs2O, HCl, HCN,
HNO\cs3, HO\cs2, HOCl, N\cs2O, O\cs3, and OH and SO\cs2, along with
temperature, geopotential height, relative humidity (deduced from the
H\cs2O and temperature data), and cloud ice water content, all described
as functions of pressure.  In \thisv these profiles are mostly output on
a grid that has a vertical spacing of six surfaces per decade change in
pressure (\texttildelow2.5\,km), thinning out to three surfaces per
decade above 0.1\,hPa.  Exceptions to this are water vapor, temperature,
ozone and relative humidity which are on a finer 12 per decade grid from
...
software is version~4.11, producing files labeled \texttt{v04-11}.  Any
...
will be referred to as v4.12, v4.13, etc.  This document is intended to
be applicable to any \thisv data files.  More substantial changes at a
later date (e.g., due to a change in the MLS instrument performance) may
necessitate a larger change in the data processing software and/or its
configuration.  These will be numbered v4.2\emph{x} etc., and will be
accompanied by an updated version of this document (though not perhaps
...
defined\footnote{See
  \href{http://science.nasa.gov/earth-science/earth-science-data/data-maturity-levels/}{\texttt{http://science.nasa.gov/earth-science/earth-science-data/data-maturity-levels/}}}
as:
\begin{quote}\itshape
...
\section{Differences between MLS \thisv data and earlier v3.3 data}
\label{sec:V3vsV2Overview}

All of the MLS data products have been updated from the earlier v3.3 and v3.4
algorithms.  Significant updates are detailed below.
\begin{description}
\item[New material] is needed.
\end{description}
...
The significant improvements in the handling of clouds in \thisv
compared to the earlier \prevv data processing means that the various
complex rules for cloud ``screening'' recommended to data users for
those earlier versions can be dispensed with.  Also note that the
threshold values of ``Quality'' and ``Convergence'' to be applied in
data screening have been updated for all products.
...
site, and one for python is planned to be added
shortly. \mycomment{Update python discussion}
X
The standard products are stored in files named according to the convention
\begin{quote}
\verb+MLS-Aura_L2GP-<product>_v04-11_<yyyy>d<ddd>.he5+
\end{quote}
...
\end{quote}
\mycomment{Need to update these!}  where \verb+<case>+ is \verb+Eq+ or
\verb+70N+ for the equator and 70\degsym N, respectively (or \verb+Day+
and \verb+Night+ for OH, see section~\ref{sec:StandardOH}).  These files
are available from the MLS web site at
...
been (or may be) reactivated, the \thisv algorithms also produce a
second HCl product (in the \texttt{HCl-640-B13} swath in the
\texttt{L2GP-DGG}) file which includes the band~13 radiances, giving a
product with improved precision and resolution in the upper stratosphere
and mesosphere.  See the section~\ref{sec:standardHCl} for more
information.
...
\mycomment{Get new discussion from Shuhui}
