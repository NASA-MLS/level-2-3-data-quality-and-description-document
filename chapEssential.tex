...
\begin{figure}[tb]
  ...
  \caption{Comparisons of MLS (v1.5) N\cs2O observations with in-situ balloon data (courtesy of J.~Elkins).  The raw balloon data  ($\b{z}_h$) are shown as the grey shaded region (shading indicates precision).  A coincident MLS profile ($\b{x}_l$) is shown in red with the red error bars indicating precision.  The red dots show the MLS data linearly interpolated to the balloon pressures using the $\b{H}$ matrix (i.e., $\b{x}_h$ from equation~\ref{eqn:EtaForward}).  The black line shows the ``least squares'' interpolation of the balloon data onto the MLS grid using the $\b{W}$ matrix as described in the text (i.e., $\b{z}_l$ from equation~\ref{eqn:EtaBackward}).  The black line therefore represents the closest possible match at this resolution to the original grey line, and is the appropriate quantity to compare to the red MLS profile, and to be multiplied by the averaging kernels for formal comparison.}
