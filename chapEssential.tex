...
\begin{quote}
\emph{The MLS algorithms identified some of the radiances in this region as being affected by clouds and opted not to use them in the retrieval of this composition/temperature profile, thus changing the character of the retrieved profile in some manner that sets it apart from those in scenes marked cloud-free.}\quad(\thisv)
\end{quote}
...
\end{quote}
where \verb+<case>+ is \verb+Eq+ or \verb+70N+ for the equator and 70\degsym{}N, respectively (or \verb+Day+ and \verb+Night+ for OH, see Section~\ref{sec:StandardOH}).  These files, for all products, are available as a single tar archive from the MLS web site at \url{https://mls.jpl.nasa.gov/eos-aura-mls/data-products/averaging-kernels/}.  \mycomment{Make sure both the 2D and 1D files are available on the MLS website, and that the link is correct.}
...
% \section{A note on HCl measurements in \thisv}
% \label{sec:Chap1HClNote}

% \mycomment{Do we still want to have this information in section 1 (same for OH and N2O)?  It is product-specific, so perhaps now belongs in with those products.  We perhaps had it here when it was first an issue because we felt it was new/important enough that it merited such high-level emphasis.   If nothing else, do we still need the ``in \thisv`` clause in the section headings?}
