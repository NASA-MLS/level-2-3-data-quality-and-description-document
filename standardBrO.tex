\speciestitle{Bromine monoxide}{BrO}{BrO}\startthumbmarks%
\begin{description}
\item[Useful range: ] 10\,--\,3.2\,hPa (day/night differences needed)
\item[Contact:] Nathaniel Livesey, \email{Nathaniel.J.Livesey@jpl.nasa.gov}
\end{description}%
\label{sec:StandardBrO}%
\subsection*{Introduction}

The standard product for BrO is taken from the 640-GHz (Core + R4A)
retrievals. The spectral signature of BrO in the MLS radiances is very
...
negligible; however, for lower pressures, nighttime BrO needs to be
...
Note, the \thisv `standard' BrO product (as with earlier versions) contain
systematic biases and horizontal oscillations that present a larger
challenge than for other species.  Those interested in using MLS BrO in
scientific studies are strongly advise to contact the MLS team before
embarking on their research.  Different algorithms for BrO are under
development by the MLS team, aimed at ameliorating some of these
artifacts.
...
\begin{figure}
\begin{center}
  \dontincludegraphics[width=0.4\linewidth]{figures/bro-precision}
\end{center}
...
modeled [see Read et al., 2007]. These quantified effects correspond to
...
where there is no diurnal variation in BrO.
...
embarking on research involving the MLS standard BrO product.  Improved
versions of the BrO product are under development at the time of
writing.
...
inferred from balloon-borne measurements of BrO obtained by the
instruments DOAS, SAOZ, and SLS. When plotted in tracer space (e.g., as
a function of N$_{2}$O), which accounts for differences in age of air,
good agreement is seen \citep{KovalenkoEtal07}.

\subsection*{Desired improvements for future data version(s)}
\begin{itemize}
\item Improvements will be sought in the stability of the BrO biases
\item Future versions will also seek to improve the quality of the BrO
  observations in the mid- and lower stratosphere
\item Improvements will also be sought in the polar regions, especially
  during summer / winter, when day / night differences are not possible
\end{itemize}
