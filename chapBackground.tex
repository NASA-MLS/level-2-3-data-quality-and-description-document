...
\rcsInfo $Id: chapBackground.tex,v 1.1 2014/08/15 17:07:41 livesey Exp $
...
spectrally varying error in radiance.  This is true of both instrumental
effects such as variations in instrument gain and offset during the limb
scan, and ``forward model'' effects such as knowledge of continuum
emission and the impact of some approximations made in the forward model
in order to increase its speed.  In order to account for such effects,
the \thisv algorithms also retrieve spectrally flat (or slowly
spectrally varying) corrections to the MLS radiances, either in terms of
an additive radiance offset or an additive atmospheric extinction.
...
contrast, a spectrally-flat baseline is used.  However, optically thick
clouds can affect the MLS radiances beyond the modeling capability of
this approach, mainly through scattering processes.  Such situations are
and the affected radiances excluded from the retrievals, or their
influence down-weighted.
...
\mycomment{Check with Bill on this next paragraph} In the case of the
240\,GHz radiometer (\texttt{R3:240}), instead of computing a \Tcir\
parameter, the fit achieved in an early retrieval phase to the
\texttt{B8F:PT} band (that measures the 233.9-GHz O\cp{18}O line), as
quantified by a $\chi^2$-metric is used as an indicator of potential
significant cloud-contamination.  In computing \Tcir\ for the other
radiometers, the forward model calculation takes the best retrieved
atmospheric state, with relative humidity capped at 110\%.
...
changes in \texttt{Quality} for a given product necessarily reflect an
improvement.
