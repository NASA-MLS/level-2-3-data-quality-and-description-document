...
The detection of cloud scattering is based on a poor lineshape fit (typically manifested as the measured linewidth being unphysically narrow compared to the calculated width) and unexpected signal enhancements/depressions in the optically thin MLS channels when viewing low in the atmosphere (limb tangent altitudes below \mytexttilde10\,km)..  The cloud detection calculation uses estimates of tangent pressure and temperature obtained from the \mbox{118\nobreakdash-GHz} Band~1 measurements (unaffected by clouds) and tangent point altitude information obtained from instrument/spacecraft pointing knowledge. These, combined with an estimate of relative humidity from a previous retrieval phase, are used to compute the radiances in the most optically thin MLS channel (\mbox{Channel~3} of \mbox{Band-33}) expected in a clear-sky scene.  These are compared to the radiances measured in that channel, with the difference giving a vertical profile of estimated cloud-induced radiances (\Tcir).  Clear-sky radiances are also computed for the O\cp{18}O signal measured in \mbox{Band-8} to produce vertical profiles of $\chi^2$. The \Tcir and $\chi^2$ are used to deduce the highest limb view affected by cloud scattering (the specific criteria are that \mbox{Band-8} $\chi^2 > 10$ or \Tcir $< -15$\,K indicate a cloud in the field of view); radiances from limb views lower than this highest cloud detection view are not used in the composition retrievals.
