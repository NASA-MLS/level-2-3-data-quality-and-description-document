\chapter{Background reading for users of MLS version~3.3 and~3.4 data}
\rcsInfo $Id: chapBackground.tex,v 1.4 2013/05/03 00:51:06 livesey Exp $
\label{chap:Background}

\section{EOS MLS radiance observations}
...
climatological fields), and constraints on the smoothness of the result.
This fit must often be arrived at in an iterative manner because of the
non-linear nature of the EOS MLS measurement system.

An innovative aspect of the retrieval algorithms for EOS MLS arises from
...
reviews the EOS MLS measurement geometry and shows that each radiance
...
measurement system manifest themselves as a spectrally flat error in
radiance.  This is true of both instrumental effects such as variations
in instrument gain and offset during the limb scan, and ``forward
model'' effects such as knowledge of continuum emission and the impact
of some approximations made in the forward model in order to increase
its speed.  In order to account for such effects, the \thisv algorithms
also retrieve spectrally flat (or slowly spectrally varying) corrections
to the MLS radiances, either in terms of an additive radiance offset or
an additive atmospheric extinction.

\section{The Core, Core+Rn approach}
...
The ``Core'', phases are followed by phases such as ``Core+R3'' and
``Core+R5'', where composition profiles are retrieved from a given
radiometer.  Sometimes (e.g., for ``Core+R3'') these later phases
...
phases (e.g., the ``Core+R2'' and ``Core+R4'' families of phases), the
...
originated.  For example, the ozone obtained from the ``Core+R2''
retrieval is known in the \thisv dataset as \texttt{O3-190}.  In \thisv in
order to reduce confusion for users of MLS data, the algorithms also
output ``standard'' products, which is typically a copy of one of the
products from the `Core+Rn' phases.  For example, the ``standard'' ozone
product is a copy of the \texttt{O3-240} product.  In the case of \thisv
nitric acid, the standard product represents a hybrid of the results
from two phases.  Details of which standard product is obtained from
which phase are given in table~\ref{tab:StandardProducts}.

\newcommand{\mybox}[2]{\parbox[m]{#1}{\raggedright #2}}
\newcommand{\phase}[1]{\mybox{1.0in}{#1}}
...
\textbf{Phase} & \textbf{Target species}\footnote{\sffamily Tangent pressure and
    ...
    focus of the phase.  Temperature, IWC, H\cs2O, RHi are `high resolution'
...
\species{T, pTan~(GHz), GPH} & 
\measurements{R1A \& R1B~(118\,GHz)} &
\tablecomments{Better P/T estimate, lower limit 261\,hPa} \\ 

\phase{Init-Ext} &
\species{240\,GHz extinction} &
\measurements{R3~(240\,GHz), single channel} &
\tablecomments{Initial R3 extinction estimate, lower limit 464\,hPa} \\ 

\rowcolor{lightgrey} 
\phase{Final-Ext} &
\species{240\,GHz extinction, O\cs3} &
\measurements{R3~(240\,GHz)} &
\tablecomments{Better R3 extinction and P/T estimate, lower limit
  261\,hPa.  Also used for cloud flagging later P/T retrievals} \\
...
\rowcolor{lightgrey} 
\phase{R3Init-RHi} &
\species{UT RHi\footnote{\sffamily RHi from the R3 phases serves as an
    extinction/baseline quantity, but expressed in percent RHi units.}} & 
\measurements{R3~(240\,GHz)} &
\tablecomments{Retrieve RHi in a \texttildelow6\,km-layer centered
  400\,--\,600\,hPa.  An initial estimate of RHi profile from an R3 window channel} \\ 

\phase{Core} &
 --- & --- & \tablecomments{Product collation, and cloud flagging} \\


\rowcolor{lightgrey} 
\phase{Core+R3} & \species{T, pTan~(GHz), GPH, O\cs3\footnote{\sffamily On high vertical resolution
...
\centering\begin{tabular}{lll}\toprule
  \textbf{Product} & \textbf{Origin} & \textbf{Spectral region}\\
  \midrule
  BrO       & Core+R4A (B14) & 640\,GHz \\
  \rowcolor{lightgrey} 
  CH\cs3Cl  & Core+R4A (B14) & 640\,GHz \\
  CH\cs3CN  & Core+R4A (B14) & 640\,GHz \\
  \rowcolor{lightgrey} 
  ClO       & Core+R4A (B14) & 640\,GHz \\
  CO        & Core+R3 & 240\,GHz \\
  \rowcolor{lightgrey} 
  H\cs2O    & Core+R2 & 190\,GHz \\
  HCl       & Core+R4A (B14) & 640\,GHz \\
  \rowcolor{lightgrey} 
  HCN       & Core+R2 & 190\,GHz \\ 
  HNO\cs3   & \mybox{5cm}{Core+R2 (15\,hPa and less) \\ Core+R3 (larger
    than 15\,hPa)} &
  \mybox{2.5cm}{190\,GHz\\240\,GHz} \\
  \rowcolor{lightgrey} 
  HO\cs2    & Core+R4A (B14) & 640\,GHz \\
  HOCl      & Core+R4A (B14) & 640\,GHz \\
  \rowcolor{lightgrey} 
  IWC       & High-Cloud & 240\,GHz \\
  IWP       & High-Cloud & 240\,GHz \\
  \rowcolor{lightgrey} 
  N\cs2O    & Core+R4B & 640\,GHz \\ 
  O\cs3     & Core+R3 & 240\,GHz \\
  \rowcolor{lightgrey} 
  OH        & Core+R5 & 2.5\,THz \\
  RHi       & \parbox[m]{1.5in}{\raggedright Computed from Temperature and H\cs2O} & 190\,GHz \\
  \rowcolor{lightgrey} 
...
This model is by its nature approximate.  Many of the biases and
unexpected scatter seen in the \thisv simulation studies can be attributed
to inaccuracies in this model.  The model accuracy is a function of the
proximity of the retrieved state to the pre-selected state used.  The
pre-selected states are taken from climatological fields for fixed
latitudes and calendar months.  In regions where the atmosphere departs
dramatically from the climatological values (e.g.,\ in the winter polar
vortices), the model will generally be poorer than in other locations,
giving rise to stronger biases.

In addition, a ``cloud'' forward model can be invoked to model the
effects of scattering from cloud particles in the troposphere and lower
stratosphere \citep{EOSMLSCloudATBD}.  This model was used in the
simulation of radiances based on known model atmospheres for the \thisv
testing, but is not invoked in the \thisv retrieval algorithms (the
handling of clouds is described in more detail in
...
Core+R3 retrieval this is handled by retrieving RHi, acting as a
frequency squared dependent extinction (including background atmospheric
absorption from N\cs2, H\cs2O and unknown emitters).  In the other
retrieval phases, by contrast, a spectrally-flat baseline is used.
However, optically thick clouds can affect the MLS radiances beyond the
modeling capability of this approach, mainly through scattering
processes.  Such situations need to be identified and the affected
radiances excluded from the retrievals, or their influence
down-weighted.

The first aspect of handling clouds in \thisv is therefore the flagging of
radiances that are believed to be significantly contaminated by cloud
...
Specific window channels (those that see deepest into the atmosphere) in
each radiometer are chosen to set these flags.

In the case of the 240\,GHz radiometer (\texttt{R3:240}), instead of
computing a \Tcir\ parameter, the fit achieved in an early retrieval
phase to the \texttt{B8F:PT} band (that measures the 233.9-GHz O\cp{18}O
line), as quantified by a $\chi^2$-metric is used as an indicator of
potential significant cloud-contamination.  In computing \Tcir\ for the
other radiometers, the forward model calculation takes the best
retrieved atmospheric state, with relative humidity capped at 110\%.

Where the \Tcir\ (or $\chi^2$ for \texttt{R3:240}) values are
sufficiently large (see Table~\ref{tab:CloudThresholds}), the radiances
are flagged as being possibly contaminated.  The estimated \Tcir\ or
$\chi^2$ are improved as the retrieval progresses through the various
phases, and finalized in the \texttt{HighCloud} phase, where \Tcir\
statistics are computed and output to a diagnostic file for a wide range
of channels including the window channels.

The retrievals of gas phase species abundances may choose to reject the
cloud contaminated radiances, or (in the case of some less impacted
channels) to inflate their estimated radiance precisions.
...
\centering\small\begin{tabular}{lllll}
\toprule
\textbf{Radiometer} & \textbf{Cloud channel} & \textbf{USB/LSB frequency
  / GHz} & 
\textbf{Low threshold} & \textbf{High threshold} \\
\midrule
%
\rowcolor{lightgrey}
R1[A/B]:118 & B[32/34]W:PT.C4 & 115.3 (LSB only) & $\Tcir <
-$4\,K & none \\
%
R2:190 & B5F:ClO.C1 & 178.8 / 204.9 & $\Tcir <
-$20\,K & $\Tcir > $10\,K \\
%
\rowcolor{lightgrey}
...
\section{A brief note on the `Quality' field}
...
reflect the species' relative validity.
