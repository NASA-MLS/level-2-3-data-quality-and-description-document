...
\rcsInfo $Id: chapBackground.tex,v 1.3 2014/08/25 18:38:55 livesey Exp $
...
\centering\begin{tabular}{lll}\toprule
  ...
  \flipfloprow 
  ClO       & CorePlusR4A (B14) & 640\,GHz \\
  \flipfloprow 
  CO        & CorePlusR3 & 240\,GHz \\
  \flipfloprow 
  H\cs2O    & CorePlusR2 & 190\,GHz \\
  \flipfloprow 
  HCl       & CorePlusR4A (B14) & 640\,GHz \\
  \flipfloprow 
  HCN       & CorePlusR2 & 190\,GHz \\ 
  \flipfloprow 
  HNO\cs3   & \mybox{5cm}{CorePlusR2 (15\,hPa and less) \\ CorePlusR3 (larger
    than 15\,hPa)} &
...
severe line shape distortions which are detectable in the Band~8
O\cp{18}O line fit, based on extrapolating a tangent pressure estimate
into the upper troposphere from the pressure information obtained from
the 118\,GHz Band~1 measurements (unaffected by clouds).  RHi from a
previous phase is used to compute a cloud induced radiance (T\cs{cir}).
The vertical profiles of $\chi^2$ and T\cs{cir} are analyzed to find the
highest limb view affected by cloud scattering, and radiances below are
handled accordingly.
