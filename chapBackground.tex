...
The Core, phases are followed by phases such as \texttt{CorePlusR3} and \texttt{CorePlusR5}, where composition profiles are retrieved from a given radiometer. Sometimes (e.g., for \texttt{CorePlusR3}) these later phases continue to retrieve temperature and pressure, continuing using information from the 118-GHz radiometers, as in the Core phases. In other phases (e.g., the \texttt{CorePlusR2} and \texttt{CorePlusR4} families of phases, the 118-GHz information is neglected and temperature and pressure are constrained to the results from the earlier Core phases. This choice is made based on extensive testing aimed at maximizing the information yield from MLS while minimizing the impact of inevitable systematic disagreements among the different radiometers, introduced by uncertain spectroscopy and/or calibration knowledge.

Table~\ref{tab:Phases} describes the phases in more detail. Many products (e.g., ozone) are produced in more than one phase. All the separate measurements of these species are produced as diagnostic quantities, and typically labeled according to the spectral region from which they originated. For example, the ozone obtained from the \texttt{CorePlusR2} retrieval is known in the \thisv dataset as \texttt{O3-190}. In \thisv, as with previous versions, in order to reduce confusion for users of MLS data, the algorithms also output ``standard products'', which are typically copies of one of the products from the \texttt{CorePlusR\emph{n}} phases. For example, the standard chlorine monoxide product is a copy of the \texttt{ClO-640} product. In the case of \thisv nitric acid, the standard product represents a hybrid of the results from two phases. Details of which standard product is obtained from which phase are given in Table~\ref{tab:StandardProducts}.
...
\flipfloprow
\phase{InitR2} &
\species{\ce{H2O}, \ce{N2O}, \ce{CH3CN}, HCN, ClO, \ce{HNO3}, \ce{O3}, SO\cs2} &
...
\flipfloprow
\phase{InitUTH} &
\species{Upper tropospheric \ce{H2O}} & 
\measurements{R2~(190\,GHz)} &
\tablecomments{Low vertical resolution (6/decade) \ce{H2O} retrieval} \\ 

\flipfloprow
\phase{CorePlusR3} & \species{T, pTan~(GHz), GPH, \ce{O3}\footnote{\sffamily On high vertical resolution grid}, CO, \ce{HNO3}, SO\cs2, RHi} &
\measurements{R1A \& R1B~(118\,GHz), R3~(240\,GHz)} &
...
\species{\ce{O3}, CO} &
\measurements{R3~(240\,GHz)} &
\tablecomments{Retrievals down to 316\,hPa optimized for \ce{O3} product}\\ 

\flipfloprow
\phase{CorePlusR2} &
\species{\ce{H2O}, \ce{N2O}, \ce{HNO3}, ClO, \ce{O3}, HCN, \ce{CH3CN}, SO\cs2, pTan~(GHz)} &
\measurements{R1A \& R1B~(118\,GHz), R2~(190\,GHz)} & 
\tablecomments{\ce{H2O} retrieved down to 316\,hPa, other species 100\,hPa (note no T, GPH, pTan retrieval)} \\ 

\flipfloprow
\phase{Hydrogen Cyanide} &
\species{HCN, \ce{O3}, \ce{HNO3}} &
\measurements{R2~(190\,GHz)} & 
\tablecomments{Retrievals down to 100\,hPa} \\ 

\flipfloprow
\phase{CorePlusR4AB14} & \species{ClO, BrO, \ce{HO2}, HOCl,
  HCl, \ce{O3}, \ce{HNO3}, \ce{CH3CN}, SO\cs2, \ce{CH3Cl}} &
\measurements{R4~(640\,GHz)} &
\tablecomments{Retrievals down to 147\,hPa}\\ 

\flipfloprow
\phase{Methanol} &
\species{\ce{CH3OH}, ClO, BrO, \ce{HO2}, HOCl, \ce{HNO3}, SO\cs2, \ce{CH3Cl}} &
\measurements{R2~(190\,GHz) and R4~(640\,GHz)} & 
\tablecomments{Retrievals down to 147\,hPa} \\ 

\flipfloprow
\phase{CorePlusR4AB13} & \species{HCl, \ce{O3}, SO\cs2} &
\measurements{R4~(640\,GHz)} &
\tablecomments{Retrievals down to 147\,hPa. This phase is only executed when
  MLS band~13 is operating}\\ 

\flipfloprow
\phase{CorePlusR4B} & \species{\ce{N2O}, SO\cs2} &
\measurements{R4~(640\,GHz)} &
...
\flipfloprow
\phase{CorePlusR5} & \species{T, pTan~(GHz,~THz), GPH, OH, \ce{O3}} &
...
\begin{table}
  ...
  \flipfloprow 
  \ce{CH3Cl}  & CorePlusR4AB14 & 640\,GHz \\
  \flipfloprow 
  \ce{CH3CN}  & CorePlusR4AB14 & 640\,GHz \\
  \flipfloprow 
  \ce{CH3OH}  & Methanol & 640\,GHz \\
  \flipfloprow 
  ...
  \flipfloprow 
  \ce{H2O}    & CorePlusR2 & 190\,GHz \\
  \flipfloprow 
  HCl       & CorePlusR4AB14 & 640\,GHz \\
  \flipfloprow 
  HCN       & HydrogenCyanide & 190\,GHz \\ 
  \flipfloprow 
  \ce{HNO3}   & \mybox{6cm}{CorePlusR2 \textsf{(15\,hPa and less)} \\ CorePlusR3 \textsf{(larger
    than 15\,hPa)}} &
  \mybox{2.5cm}{190\,GHz\\240\,GHz} \\
  \flipfloprow 
  \ce{HO2}    & CorePlusR4AB14 & 640\,GHz \\
  \flipfloprow 
  ...
  \flipfloprow 
  \ce{N2O}    & CorePlusR2 & 190\,GHz \\ 
  \flipfloprow 
  \ce{O3}     & OzoneOnly & 240\,GHz \\
  \flipfloprow 
  OH        & CorePlusR5 & 2.5\,THz \\
  \flipfloprow 
  RHi       & \mybox{6cm}{\sffamily\raggedright Computed from Temperature and \ce{H2O}} & 190\,GHz \\
  \flipfloprow 
...
\section{Changes in \thisv retrievals of \texorpdfstring{\ce{N2O}}{N2O}}
\label{sec:V6N2OCorrectionDetails}

As discussed in Section~\ref{sec:V6N2OCorrectionBriefSummary}, changes were made in the \thisv algorithms to the manner in which \ce{N2O} is retrieved from radiances measured by the MLS 190-GHz radiometer.  MLS observes \ce{N2O} by measuring a spectral line that is spectrally close to (particularly in the folded-sideband MLS measurements), but 25 times weaker than, the \ce{H2O} line at 183\,GHz. The sideband fraction fix developed for the CorePlusR2 products in \prevv clearly improved \ce{H2O} (and \ce{O3} measurements made by the 190-GHz receiver) but, for unknown reasons, degraded \ce{N2O}. As an unsatisfying work around for this issue, \ce{N2O} is \prevv was retrieved from a separate phase where the sideband fractions were based on the prelaunch calibration value (in contrast with \ce{H2O}, which was retrieved using a corrected value for this fraction), with a correction for the drift. This phase also unrealistically assumed no contribution to the 190-GHz signals from continuum emission.  With the exception of drift correction, these changes were empirically derived and non-physical \citep{LiveseyEtal:2021:Drifts}.  For \thisv \ce{N2O} we opted to instead determine a correction for the 190-GHz radiances based on \ce{N2O} information obtained from the stronger and less interference-prone 640-GHz line (measurements from which were available from launch through August 2013). Using a subset of the days for which 640-GHz \ce{N2O} measurements are available, we examined the spectral and scan-angle-dependent form of the correction needed to improve the agreement between the 190- and 640-GHz \ce{N2O} products.  In order to reduce the dependence on specifics of the MLS scan etc., we opted to characterize the correction as a function of the total integrated radiance measured across the 190-GHz band, as a proxy for limb tangent pressure and other parameters.  With this correction in place, a separate phase for retrieving \ce{N2O} is no longer needed, and the standard \ce{N2O} product is again retrieved concurrently with \ce{H2O} and \ce{HNO3} (for pressure $\le$ 14.7\,hPa) from the CorePlusR2 phase, as in \viv and previous versions. The root of the issue is that the interference between the strong \ce{H2O} line and the weak \ce{N2O} signature is not modeled well enough to provide an accurate \ce{N2O} retrieval (e.g., to within 10\%), especially in the lower stratosphere and upper troposphere.
...
Although the term ``systematic uncertainty'' is often associated with consistent biases and/or scaling errors, many sources of ``systematic'' error in the MLS measurement system give rise to additional scatter. For example, an error in the \ce{O3} spectroscopy, while being a constant bias on the fundamental parameter, will have an impact on the retrievals of species with weaker signals (e.g., CO) that is dependent on the amount and morphology of atmospheric ozone. The extent to which such terms can be expected to average down is estimated to first order by these ``full up studies'' through their separate consideration of the bias and scatter each uncertainty source introduces.
