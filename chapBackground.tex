...
above the \mytexttilde126\,GHz local oscillator) are suppressed.  These
...
of antenna retrace and calibration activity.  This \mytexttilde25\,s cycle is
known as a \emph{Major Frame} (MAF).  During the \mytexttilde20\,s continuous
...
retrieval approach \citep{Rodgers:1976,Rodgers:2000} that seeks the ``best''
...
consecutive profiles.  In the \thisv\CheckThis Level~2 algorithms, the state vector
...
\thisv\CheckThis algorithms also retrieve spectrally flat (or slowly spectrally
...
\emph{phases}.  In the case of the MLS \thisv\CheckThis retrievals, there are many
...
modeled) cloud scattering.  Such minor frames are ignored in \thisv\CheckThis
...
\texttt{CorePlusR2} retrieval is known in the \thisv\CheckThis dataset as
\texttt{O3-190}.  In \thisv\CheckThis in order to reduce confusion for users of
...
the case of \thisv\CheckThis nitric acid, the standard product represents a hybrid
...
\tableheadentry{Phase} & \tableheadentry{Target species\footnote{\sffamily Tangent pressure and
    ...
    Core+R3 phase.}} & \tableheadentry{Measurements} &
...
The retrieval algorithms in \thisv\CheckThis make use of a variety of different
...
This model is generally rather time consuming, although for some
comparatively ``clean'' spectral regions the computational burden is
small enough that the full forward model can be used in the
operational retrievals.  Further, in the years since the Aura launch,
computational capabilities have improved significantly, allowing for
wider use of the full forward model in the MLS retrievals.  

For some of the MLS channels, a simpler ``Linearized'' forward model
can be used to estimate either radiances, or more typically the
``Jacobian'' (also know as ``weighing function'') information.  The
Linearized model invokes a simple first-order Taylor series to
estimate radiances as a function of the deviation of the state from
one of several pre-selected representative states.  The inputs to this
model are pre-computed radiances and derivatives corresponding to the
pre-selected states, generated by ``off-line'' runs of the full
forward model.

In addition, a ``cloud'' forward model can be invoked to model the
effects of scattering from cloud particles in the troposphere and
lower stratosphere \citep{EOSMLSCloudATBD}.  This model was used in
the simulation of radiances based on known model atmospheres for the
\thisv\CheckThis testing, but is not invoked in the \thisv\CheckThis retrieval algorithms
(the handling of clouds is described in more detail in
...
than the wavelengths of the radiation being observed. The MLS \thisv\CheckThis
...
The first aspect of handling clouds in \thisv\CheckThis is therefore the flagging
...
The other aspect of cloud handling in \thisv\CheckThis is the estimation of cloud
...
A major component of the validation of MLS data is the quantification
of the various sources of systematic uncertainty.  These can arise
from instrumental issues (e.g., radiometric calibration, field of view
characterization), spectroscopic uncertainty, and through
approximations in the retrieval formulation and implementation.  A
comprehensive quantification of these uncertainties has been performed
for the older \prevv\CheckThis\ MLS products, updated from that described in the
MLS validation papers (which were based on v2.2).  The individual
sections of Chapter~\ref{chap:Results} detail the results of this
quantification product-by-product.  We do not expect the estimated
systematic errors in the MLS products to vary significantly between
\thisv\CheckThis\ and \prevv\CheckThis; however, we do plan to update these estimates for
\thisv\CheckThis\ and report findings in an updated version of this document.
...
of \citet{ReadEtal:2007} gives more specific details of the perturbations
used in the study.
...
knowledge of spectroscopy and/or instrument calibration, the \thisv\CheckThis
